        ...
        @forelse($products as $product)
            <article class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                @if($product->featured_image_url)
                    <img src="{{ $product->featured_image_url }}" alt="{{ $product->name }}" class="w-full h-48 object-cover" {!! media_lazy_attr() !!}>
                @endif
                <div class="p-6">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                        <a href="{{ url('/products/' . $product->slug) }}" class="hover:text-blue-600 dark:hover:text-blue-400">
                            {{ $product->name }}
                        </a>
                    </h2>
