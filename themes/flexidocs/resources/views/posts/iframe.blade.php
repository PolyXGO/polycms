...
@section('sidebar')
    @php
        ...
        if (isset($category)) {
            ...
            // Build the ancestry chain up to the flexidocs/root ancestor
            $ancestors = collect([]);
            $current = $cat;
            $flexidocs_root = null;
            while ($current) {
            ...
            if ($flexidocs_root) {
                // Step down to direct child of root that's in the ancestry chain (2nd level)
                $scoped = $ancestors->first(function ($c) use ($flexidocs_root) {
                    return $c->parent_id == $flexidocs_root->id;
                });
