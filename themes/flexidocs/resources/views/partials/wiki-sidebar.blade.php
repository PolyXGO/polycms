            ...
            @if($posts->isNotEmpty())
                <ul class="space-y-1">
                    @foreach($posts as $post)
                        ...
                        <li>
                            <a href="{{ $articleUrl }}" 
                               class="group block px-3 py-2 rounded-md text-sm transition-colors {{ $isActive ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 font-medium' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/50 dark:hover:text-white' }}">
                                <span class="block">{{ $post->title }}</span>
                                @if($subtitle)
                                    <span class="block text-xs mt-0.5 {{ $isActive ? 'text-indigo-400 dark:text-indigo-500' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-400' }}">
                                        {{ $subtitle }}
                                    </span>
                                @endif
                            </a>
