<article class="prose prose-slate dark:prose-invert max-w-none mt-2 lg:mt-4">
    <div class="wiki-content prose-headings:scroll-mt-24 prose-a:text-indigo-600 dark:prose-a:text-indigo-400 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl">
        <div class="mb-6 pb-4 border-b border-slate-200 dark:border-slate-700">
            @if($category = $post->categories->first())
                <nav class="flex text-sm text-slate-500 dark:text-slate-400 mb-4 items-center">
                    <a href="{{ $category->frontend_url ?? url('/categories/' . $category->slug) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                        {{ $category->name }}
                    </a>
