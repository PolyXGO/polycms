                ...
                <ul class="space-y-2.5 text-slate-600 dark:text-slate-400">
                    <template x-for="(heading, index) in headings" :key="heading.id">
                        <li :class="{ 'ml-4': heading.level === 3, 'ml-8': heading.level === 4, 'font-medium text-slate-900 dark:text-slate-200': activeId === heading.id }">
                            <a @click.prevent="scrollToHeading(heading.id, index)" :href="'#' + heading.id" x-text="heading.text" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors" :class="{'text-indigo-600 dark:text-indigo-400': activeId === heading.id}"></a>
                        </li>
                    </template>
                ...
                <div class="mb-8">
                    ...
                    <ul class="space-y-2">
                        @foreach($directPosts as $post)
                            <li>
                                <a href="{{ $post->frontend_url }}" class="text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                    {{ $post->title }}
                                </a>
                            </li>
                        @endforeach
                ...
                @foreach($childCategories as $childCategory)
                    <div class="mb-8">
                        <h2 class="text-sm font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">
                            <a href="{{ $childCategory->frontend_url }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">{{ $childCategory->name }}</a>
