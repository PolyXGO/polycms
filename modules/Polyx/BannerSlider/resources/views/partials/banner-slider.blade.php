            ...
            <div class="banner-slider" id="banner-slider">
                @foreach($banners as $banner)
                    <div class="banner-slide" data-type="{{ $banner['type'] ?? 'image' }}" data-banner-id="{{ $banner['id'] ?? '' }}"
                         style="@if(isset($banner['padding_x'])) --banner-padding-x: {{ $banner['padding_x'] }}px; @endif @if(isset($banner['padding_y'])) --banner-padding-y: {{ $banner['padding_y'] }}px; @endif">
                                ...
                                <div class="banner-text-content">
                                    ...
                                    @if(!empty($banner['countdown_enabled']) && !empty($banner['countdown_date']))
                                        <div class="banner-countdown" 
                                             data-countdown-date="{{ $banner['countdown_date'] }}"
                                             data-countdown-repeat="{{ $banner['countdown_repeat'] ?? 'none' }}"
                                             data-countdown-duration="{{ $banner['countdown_duration'] ?? 0 }}">
                            ...
                            @if(!empty($banner['link']))
                                <a href="{{ $banner['link'] }}" target="{{ $banner['link_target'] ?? '_self' }}" @if(!empty($banner['link_rel'])) rel="{{ $banner['link_rel'] }}" @endif class="banner-link">
                                    <img
                                        ...
                                        loading="lazy"
                                    >
                                    @if(!empty($banner['title']) || (!empty($banner['countdown_enabled']) && !empty($banner['countdown_date'])))
                        ...
                        {{-- Admin Edit Icon - Rendered but hidden by default unless server-side admin or client-side shown --}}
                        @if(!empty($banner['id']))
                            @php
