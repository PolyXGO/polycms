        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            ...
            <div class="mb-6">
                ...
                <input
                    ...
                    id="link"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                    placeholder="e.g. https://example.com (Main link when clicking the banner)"
                />
            ...
            <div v-if="form.type === 'text'" class="mb-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div class="mb-4" v-if="form.button_text">
                    ...
                    <input
                        ...
                        id="button_link"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        placeholder="e.g. https://example.com (Specific link when clicking the button)"
                    />
            ...
            <div class="mb-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Padding Settings</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="padding_x" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Left / Right Padding (px)
                        </label>
                        <div class="flex gap-4 items-center">
                            <input
                                v-model.number="form.padding_x"
                                type="range"
                                id="padding_x"
                                min="0"
                                max="150"
                                class="flex-1"
                            />
                            <span class="text-sm text-gray-600 dark:text-gray-400 w-12 text-right">{{ form.padding_x !== null && form.padding_x !== '' ? form.padding_x : 32 }}px</span>
                        </div>
                    </div>
                    <div>
                        <label for="padding_y" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Top / Bottom Padding (px)
                        </label>
                        <div class="flex gap-4 items-center">
                            <input
                                v-model.number="form.padding_y"
                                type="range"
                                id="padding_y"
                                min="0"
                                max="100"
                                class="flex-1"
                            />
                            <span class="text-sm text-gray-600 dark:text-gray-400 w-12 text-right">{{ form.padding_y !== null && form.padding_y !== '' ? form.padding_y : 32 }}px</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="form.type === 'text'" class="mb-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div v-if="form.countdown_enabled" class="space-y-4">
                    <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="countdown_repeat" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Repeat Type
                            </label>
                            <select
                                v-model="form.countdown_repeat"
                                id="countdown_repeat"
                                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                            >
                                <option value="none">No Repeat</option>
                                <option value="hourly">Hourly Repeat</option>
                                <option value="daily">Daily Repeat</option>
                            </select>
                        </div>
                        <div>
                            <label for="countdown_duration" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            ...
                            <input
                                ...
                                min="0"
                                placeholder="e.g. 20 (0 or empty = always active)"
                                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                            />
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Starts counting down only within this many minutes before target time.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="banner-preview-container">
                    <div v-if="form.type === 'image'" class="banner-preview-image">
                        <div v-if="selectedImage" class="relative">
                            ...
                            <div v-if="form.title || form.description" class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent rounded-b-lg" :style="overlayPreviewStyle">
                    ...
                    <div v-else class="banner-preview-text" :style="previewStyle">
                        <div class="banner-preview-content">
                            ...
                            <div v-if="form.countdown_enabled && form.countdown_date && !countdownValues.isInactive" class="banner-preview-countdown" ref="countdownPreviewRef">
...
function updateCountdownPreview() {
    if (!form.value.countdown_enabled || !form.value.countdown_date) {
        countdownValues.value = { days: '00', hours: '00', minutes: '00', seconds: '00', isInactive: false };
    ...
    let target = new Date(form.value.countdown_date).getTime();
    const now = new Date().getTime();
    const repeatType = form.value.countdown_repeat || 'none';
    const duration = parseInt(form.value.countdown_duration || 0);
    ...
    if (duration > 0) {
        ...
        if (distance > durationMs) {
            countdownValues.value = { days: '00', hours: '00', minutes: '00', seconds: '00', isInactive: true };
            return;
        }
    }
...
const previewStyle = computed(() => {
    ...
    const paddingX = form.value.padding_x !== undefined && form.value.padding_x !== null && form.value.padding_x !== '' ? form.value.padding_x : 32;
    const paddingY = form.value.padding_y !== undefined && form.value.padding_y !== null && form.value.padding_y !== '' ? form.value.padding_y : 32;
...
const overlayPreviewStyle = computed(() => {
    const style: any = {};
    const paddingX = form.value.padding_x !== undefined && form.value.padding_x !== null && form.value.padding_x !== '' ? form.value.padding_x : 16;
    const paddingY = form.value.padding_y !== undefined && form.value.padding_y !== null && form.value.padding_y !== '' ? form.value.padding_y : 16;
    style.padding = `${paddingY}px ${paddingX}px`;
    return style;
});
...
const form = ref({
    ...
    countdown_date: '',
    countdown_repeat: 'none',
    countdown_duration: 0,
    padding_x: null as number | null,
    padding_y: null as number | null,
});
...
const loadBanner = async () => {
    ...
    try {
        ...
        form.value = {
            ...
            countdown_repeat: banner.countdown_repeat ?? 'none',
            countdown_duration: banner.countdown_duration ?? 0,
            padding_x: banner.padding_x ?? null,
            padding_y: banner.padding_y ?? null,
        };
...
const saveBanner = async () => {
    ...
    try {
        const payload: any = {
            ...
            countdown_repeat: form.value.countdown_repeat || 'none',
            countdown_duration: form.value.countdown_duration || 0,
            padding_x: form.value.padding_x !== null && form.value.padding_x !== '' ? parseInt(form.value.padding_x as any) : null,
            padding_y: form.value.padding_y !== null && form.value.padding_y !== '' ? parseInt(form.value.padding_y as any) : null,
        };
...
watch([
    () => form.value.countdown_enabled,
    () => form.value.countdown_date,
    () => form.value.countdown_repeat,
    () => form.value.countdown_duration
], () => {
    if (countdownInterval) {
        clearInterval(countdownInterval);
        countdownInterval = null;
