...
class SitemapService
{
    ...
    protected function buildUrlSetXml(array $items): string
    {
        ...
        foreach ($items as $item) {
            ...
            // Alternate link extensions
            if (!empty($item['alternates']) && is_array($item['alternates'])) {
                foreach ($item['alternates'] as $alt) {
                    $altHreflang = htmlspecialchars(trim((string) ($alt['hreflang'] ?? '')), ENT_QUOTES, 'UTF-8');
                    $altHref = htmlspecialchars(trim((string) ($alt['href'] ?? '')), ENT_QUOTES, 'UTF-8');
                    if ($altHreflang !== '' && $altHref !== '') {
                        $xml[] = '    <xhtml:link rel="alternate" hreflang="' . $altHreflang . '" href="' . $altHref . '" />';
                    }
                }
            }
