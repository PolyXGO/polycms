...
class SeoRenderer implements SeoRenderContract
{
    ...
    public function renderHead(): string
    {
        ...
        $description = $this->sanitizeMetaContent((string) ($payload['description'] ?? ''));
        if ($description !== '') {
        ...
        foreach (($payload['og'] ?? []) as $key => $value) {
            ...
            $metaValue = $metaKey === 'description' || $metaKey === 'title' 
                ? $this->sanitizeMetaContent((string) $value) 
                : trim((string) $value);
    ...
    /**
     * Clean up meta content by stripping HTML tags and normalizing whitespaces
     */
