<template>
    <div>
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Sample Module Settings</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Configure the Sample Module behavior</p>
            </div>
        </div>
        ...
        <form v-else @submit.prevent="saveSettings" class="space-y-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="additional_content" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Additional Content
                        </label>
                        <textarea
...
const loadSettings = async () => {
    loading.value = true;
    try {
        const response = await axios.get('/admin/sample-module/settings');
...
const saveSettings = async () => {
    saving.value = true;
    try {
        const response = await axios.post('/admin/sample-module/settings', form.value);
