...
class Google2FAServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        // Intercept web login flow (prevent 2FA bypass)
        Hook::addFilter('auth.web.authenticated', function ($response, $user, $request) {
            if ($user->hasAnyRole(['admin', 'editor', 'author'])) {
                ...
                if ($settings && $settings->google2fa_enabled) {
                    ...
                    return \Inertia\Inertia::location(url('/admin/login?error=' . urlencode('Please use the admin login page to verify Two-Factor Authentication.')));
                }
            }
            return $response;
        }, 10, 3);
