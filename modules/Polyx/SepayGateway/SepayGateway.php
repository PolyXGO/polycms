...
class SepayGateway implements PaymentGatewayInterface
{
    ...
    public function createPayment(Order $order): array
    {
        ...
        $orderCurrency = strtoupper($order->currency ?? 'VND');

        if ($orderCurrency !== 'VND') {
            if ($orderCurrency === 'USD') {
                $rate = (float) ($this->config['usd_vnd_rate'] ?? 25400.0);
                if ($rate <= 0) {
                    $rate = 25400.0;
                }
                $amount = round($amount * $rate);
            } else {
                \Illuminate\Support\Facades\Log::warning("SePay Gateway: Unsupported currency {$orderCurrency} for conversion. Defaulting to face value.");
            }
        }
    ...
    public function handleWebhook(Request $request): array
    {
        ...
        // Trigger hook to run OrderFulfillmentService (License & Sub generation)
        try {
            \App\Facades\Hook::doAction('order_status_updated', $order, $oldStatus, 'completed');
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error("SePay Webhook Hook Action Failed: " . $e->getMessage());
        }
    ...
    protected function matchOrder(string $code, string $content, float $amount): ?Order
    {
        ...
        // Try to find by order number (include 'unpaid' default status)
        $orders = Order::where('payment_method', 'sepay')
            ->whereIn('payment_status', ['pending', 'awaiting_payment', 'unpaid'])
            ->orderBy('created_at', 'desc')
            ->limit(50)
            ->get();

        foreach ($orders as $order) {
            $orderNumberClean = str_replace('-', '', strtoupper($order->code));
            ...
            if (!empty($cleanCode) && $orderNumberClean === $cleanCode) {
                ...
                if ($orderCurrency !== 'VND') {
                    if ($orderCurrency === 'USD') {
                        ...
                        $orderTotal = round($orderTotal * $rate);
                    }
                }
