...
(function() {
    ...
    if (!slider) return;
    const hasMultipleSlides = slider.children.length > 1;
    ...
    if (!showNavigation || !hasMultipleSlides) {
    ...
    if (autoSlide && hasMultipleSlides) {
        autoplayInterval = setInterval(nextSlide, autoSlideInterval);
    ...
    function initAllCountdowns() {
        ...
        countdownElements.forEach((countdownEl) => {
            if (countdownEl.dataset.countdownInitialized) return;
            countdownEl.dataset.countdownInitialized = 'true';
            ...
            let target;
            try {
                // Handle space/T separators for Safari/mobile compatibility
                const cleanDate = countdownDate.includes(' ') ? countdownDate.replace(' ', 'T') : countdownDate;
                target = new Date(cleanDate).getTime();
            } catch (e) {
                console.warn('Failed to parse date:', countdownDate, e);
                return;
            }

            if (isNaN(target)) {
                console.warn('Invalid date:', countdownDate);
                return;
            }

            const repeatType = countdownEl.dataset.countdownRepeat || 'none';
            const duration = parseInt(countdownEl.dataset.countdownDuration || 0);

            function updateCountdown() {
                const now = new Date().getTime();
                
                // Recalculate target if repeat is enabled and target is in the past
                if (repeatType !== 'none') {
                    while (target < now) {
                ...
                if (duration > 0) {
                    ...
                    if (distance > durationMs) {
                        // Hide countdown container when outside active window
                        countdownEl.style.display = 'none';
                        return;
                    } else {
                        // Show countdown container (default flex)
