...
const localAuthUser = ref<any>(null);
const authUser = computed<any>(() => props.user || page.props.auth?.user || localAuthUser.value);
...
const selectAddress = (addr: any) => {
    form.value.billing_address.full_name = addr.full_name || authUser.value?.name || '';
    ...
    form.value.billing_address.country = addr.country || 'US';
...
const fillUserData = (user: any) => {
    if (user) {
        form.value.billing_address.full_name = user.name || user.full_name || '';
        form.value.customer_email = user.email || '';
        // Logic to extract address from user.addresses if available
         if (user.addresses && user.addresses.length > 0) {
             const addr = user.addresses.find((a: any) => a.is_default) || user.addresses[0];
             if(addr) {
                 form.value.billing_address.address_line = addr.address_line || addr.line1 || '';
                 form.value.billing_address.city = addr.city || '';
                 form.value.billing_address.postal_code = addr.postal_code || '';
...
onMounted(async () => {
    // Check if we need to fetch user from token (Admin SPA user)
    if (!props.user && !page.props.auth?.user) {
        ...
        if (authToken) {
            try {
                ...
                localAuthUser.value = response.data.data || response.data;
                fillUserData(localAuthUser.value);
            } catch (e) {
                console.error('Failed to fetch auth user from token', e);
            }
        }
    } else {
        // Pre-fill address and email if user exists via props
        fillUserData(authUser.value);
    }
...
watch(isFreeOrder, (newIsFree) => {
    if (newIsFree) {
        form.value.payment_gateway = 'free';
        if (errors.value.payment_gateway) {
            delete errors.value.payment_gateway;
        }
    } else if (form.value.payment_gateway === 'free' && props.gateways && props.gateways.length > 0) {
        form.value.payment_gateway = props.gateways[0].code;
    }
}, { immediate: true });
...
const placeOrder = async () => {
    ...
    if (!isFreeOrder.value && !form.value.payment_gateway) {
        errors.value.payment_gateway = 'Please select a payment method';
        return;
    }
    
    if (!authUser.value && !form.value.customer_email) {
    ...
    try {
        ...
        const response = await axios.post('/api/v1/checkout/process', {
            ...
            payment_gateway: isFreeOrder.value ? 'free' : form.value.payment_gateway,
        ...
        } else if (response.data.payment_result && response.data.payment_result.redirect_url) {
            // Offsite redirect (e.g. PayPal / Stripe checkout or Free Order completion)
            window.location.href = response.data.payment_result.redirect_url;
        } else {
            // Normal redirect
...
const getStatusPollUrl = (orderCode: string) => {
    const pathParts = window.location.pathname.split('/');
    const activeLocales = ['vi', 'zh'];
    const currentLocale = pathParts[1];
    if (activeLocales.includes(currentLocale)) {
        return `/${currentLocale}/api/v1/orders/check-status/${orderCode}`;
    }
    return `/api/v1/orders/check-status/${orderCode}`;
};

watch(showQrModal, (newVal) => {
    if (newVal && createdOrder.value) {
        pollInterval = setInterval(async () => {
            try {
                const response = await axios.get(getStatusPollUrl(createdOrder.value.code));
            ...
            <form @submit.prevent="placeOrder">
                ...
                <section class="lg:col-span-7">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 mb-6">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-lg font-medium text-gray-900 dark:text-white">Billing Details</h2>
                            <button 
                                v-if="authUser && authUser.addresses && authUser.addresses.length > 0"
                                type="button" 
                                @click="showAddressModal = true" 
                                class="text-xs font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white underline cursor-pointer"
                            >
                        ...
                        <div class="space-y-4">
                            <div v-if="!authUser">
                                ...
                                <TextInput id="customer_email" v-model="form.customer_email" type="email" class="mt-1 block w-full" required :disabled="processing" />
                                <InputError :message="errors.customer_email" class="mt-2" />
                            </div>

                            <div>
                                <InputLabel for="full_name" value="Full Name" />
                                <TextInput id="full_name" v-model="form.billing_address.full_name" type="text" class="mt-1 block w-full" required :disabled="processing" />
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 mb-6">
                         ...
                         <div v-if="isFreeOrder" class="p-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50 rounded-xl flex items-center gap-3">
                             ...
                             <div>
                                 <h4 class="text-sm font-bold text-emerald-900 dark:text-emerald-200">Free Order - No Payment Required</h4>
                                 <p class="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5">This order is 100% free. Click "Place Order" to complete checkout instantly.</p>
                             </div>
                         </div>

                         <div v-else class="space-y-4">
                            <div v-for="gateway in props.gateways" :key="gateway.code" class="flex items-center">
                                <input :disabled="processing" v-model="form.payment_gateway" :value="gateway.code" :id="gateway.code" name="payment_method" type="radio" class="h-4 w-4 border-gray-300 text-gray-900 focus:ring-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:focus:ring-offset-gray-800" />
                ...
                <section class="lg:col-span-5">
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                        ...
                        <!-- Submit -->
                        <div class="mt-6">
                            <button @click="placeOrder" :disabled="processing || cart.isEmpty" type="button" class="w-full bg-gray-900 border border-transparent rounded-md shadow-sm py-3 px-4 text-base font-medium text-white hover:bg-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-white dark:focus:ring-gray-400 disabled:opacity-50 transition-colors">
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button @click="applyCoupon(selectedCoupon.code); closeCouponModal()" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-gray-900 text-base font-medium text-white hover:bg-black focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                            Apply Coupon
                        </button>
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 text-center">
                        ...
                        <div class="text-sm text-gray-600 dark:text-gray-300 space-y-2 mb-6">
                            <p>Amount: <span class="font-bold text-lg text-gray-900 dark:text-white">{{ formatCurrency(qrData.amount, qrData.currency) }}</span></p>
        ...
        <Modal :show="showAddressModal" @close="showAddressModal = false">
            <div class="p-6">
                ...
                <div class="space-y-4 max-h-96 overflow-y-auto">
                    <div 
                        v-for="addr in authUser?.addresses" 
                        :key="addr.id"
                        @click="selectAddress(addr)"
                        class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 cursor-pointer hover:border-gray-900 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors relative"
                        :class="{'border-gray-900 bg-gray-100 dark:border-gray-400 dark:bg-gray-700/50': form.billing_address.address_line === (addr.address_line || addr.line1)}"
                    >
                        <div v-if="addr.is_default" class="absolute top-2 right-2 text-xs font-semibold text-gray-900 dark:text-gray-300">
                            Default
                        </div>
                        <p class="font-medium text-gray-900 dark:text-white">{{ addr.full_name || authUser?.name }}</p>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ addr.phone }}</p>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ addr.address_line || addr.line1 }}, {{ addr.city }}</p>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ addr.country }}</p>
                    </div>
                </div>

                <div class="mt-6 flex justify-end">
                    <SecondaryButton @click="showAddressModal = false">
                        Cancel
                    </SecondaryButton>
                </div>
            </div>
