<template>
 ...
 <AdminLoadingState v-if="loading" :message="$t('Loading reviews...')" />

 <!-- Reviews List -->
 <div v-else class="reviews-list">
 <div v-if="reviews.length === 0" class="reviews-list__empty">
 ...
 <p class="text-admin-theme-text-muted">{{ $t('No reviews found') }}</p>
 </div>
 ...
 <!-- Pagination -->
 <div v-if="!loading && pagination.lastPage > 1" class="reviews-pagination">
...
.reviews-page__title { font-size: 1.75rem; font-weight: 700; color: rgb(var(--admin-theme-text)); }
:root.dark .reviews-page__title { color: rgb(var(--admin-theme-text)); }

.filter-select { padding: 0.5rem 0.75rem; border: 1px solid rgb(var(--admin-theme-border)); border-radius: 0.5rem; background: rgb(var(--admin-theme-surface)); color: rgb(var(--admin-theme-text)); font-size: 0.875rem; }
:root.dark .filter-select { background: rgb(var(--admin-theme-surface)); color: rgb(var(--admin-theme-text)); border-color: rgb(var(--admin-theme-border)); }

.reviews-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; }
.stat-card { padding: 1rem 1.25rem; border-radius: 0.75rem; background: rgb(var(--admin-theme-surface)); border: 1px solid rgb(var(--admin-theme-border)); display: flex; flex-direction: column; gap: 0.25rem; }
:root.dark .stat-card { background: rgb(var(--admin-theme-surface)); border-color: rgb(var(--admin-theme-border)); }
.stat-card__value { font-size: 1.5rem; font-weight: 700; color: rgb(var(--admin-theme-text)); }
:root.dark .stat-card__value { color: rgb(var(--admin-theme-text)); }
.stat-card__label { font-size: 0.75rem; color: rgb(var(--admin-theme-text-secondary)); text-transform: uppercase; letter-spacing: 0.05em; }
...
.reviews-list { background: rgb(var(--admin-theme-surface)); border: 1px solid rgb(var(--admin-theme-border)); border-radius: 0.75rem; overflow: hidden; }
:root.dark .reviews-list { background: rgb(var(--admin-theme-surface)); border-color: rgb(var(--admin-theme-border)); }
...
.reviews-table th { padding: 0.75rem 1rem; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: rgb(var(--admin-theme-text-secondary)); border-bottom: 1px solid #e2e8f0; }
:root.dark .reviews-table th { color: rgb(var(--admin-theme-text-muted)); border-color: rgb(var(--admin-theme-border)); }
.reviews-table td { padding: 0.75rem 1rem; border-bottom: 1px solid #f1f5f9; font-size: 0.875rem; color: rgb(var(--admin-theme-text)); }
:root.dark .reviews-table td { border-color: #1e293b; color: #e2e8f0; }
.reviews-table tr:hover td { background: rgb(var(--admin-theme-base)); }
:root.dark .reviews-table tr:hover td { background: rgb(var(--admin-theme-base)); }

.review-product-name { color: rgb(var(--admin-theme-primary)); font-weight: 500; text-decoration: none; }
...
.review-customer__email { font-size: 0.75rem; color: rgb(var(--admin-theme-text-muted)); }
...
.review-content__text { color: rgb(var(--admin-theme-text-secondary)); font-size: 0.8125rem; line-height: 1.4; }
.review-verified { display: inline-flex; align-items: center; gap: 0.25rem; font-size: 0.6875rem; color: #10b981; margin-top: 0.25rem; }
.review-date { font-size: 0.8125rem; color: rgb(var(--admin-theme-text-secondary)); white-space: nowrap; }
...
:root.dark .reviews-pagination { border-color: rgb(var(--admin-theme-border)); }
.pagination-btn { padding: 0.375rem 0.75rem; border: 1px solid rgb(var(--admin-theme-border)); border-radius: 0.375rem; background: rgb(var(--admin-theme-surface)); color: rgb(var(--admin-theme-text)); cursor: pointer; font-size: 0.875rem; }
.pagination-btn:disabled { opacity: 0.4; cursor: not-allowed; }
:root.dark .pagination-btn { background: rgb(var(--admin-theme-surface)); color: #e2e8f0; border-color: rgb(var(--admin-theme-border)); }
.pagination-info { font-size: 0.875rem; color: rgb(var(--admin-theme-text-secondary)); }
</style>
