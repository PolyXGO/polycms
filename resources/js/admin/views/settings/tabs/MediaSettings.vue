<template>
 ...
 <!-- Driver Section -->
 <div>
 <label for="media_driver" class="block text-sm font-medium text-admin-theme-text-secondary mb-1">
 {{ t('Driver') }}
 </label>
 <select
 id="media_driver"
 ...
 <div class="space-y-4 pt-4 border-t border-admin-theme-border">
 <div>
 <FormCheckbox
 ...
 />
 <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">
 ...
 />
 <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">{{ t('Convert file names to UUID for better security and to prevent duplicates.') }}</p>
 </div>
 ...
 />
 <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">{{ t('Prevent images from being resized or compressed during upload.') }}</p>
 </div>
 </div>

 <!-- Image Quality Settings -->
 <div class="pt-4 border-t border-admin-theme-border">
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-3">
 {{ t('Image quality') }}
 </label>
 <div class="mb-5">
 <RangeSlider 
 ...
 {{ t('Set image encoding quality (1-100). Lower values produce smaller files.') }}
  ...
  <div class="space-y-4 pt-4 border-t border-admin-theme-border">
    <div>
      ...
      />
      <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">{{ t('Automatically convert JPG and PNG images to WebP format during upload.') }}</p>
    </div>

    <div>
      <FormCheckbox
        name="media_enable_lazy_loading"
        :modelValue="getValue('media_enable_lazy_loading', true)"
        @update:modelValue="updateValue('media_enable_lazy_loading', $event)"
        :label="t('Enable Lazy Loading for images')"
      />
      <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">{{ t('Defer loading of off-screen images until the user scrolls near them for optimal page speed.') }}</p>
    </div>
  </div>

 <!-- Default Placeholder Image -->
 <div class="pt-4 border-t border-admin-theme-border">
 <label for="media_default_placeholder" class="block text-sm font-medium text-admin-theme-text-secondary mb-1">
 ...
 <div class="space-y-6 pt-4 border-t border-admin-theme-border">
    <!-- Allowed File Types -->
    <div>
      <label for="media_allowed_file_types" class="block text-sm font-medium text-admin-theme-text-secondary mb-1">
        {{ t('Allowed file types') }}
      </label>
      <input
        id="media_allowed_file_types"
        :value="getValue('media_allowed_file_types', '.png,.jpg,.jpeg,.pdf,.doc,.docx,.xls,.xlsx,.zip,.rar,.txt')"
        @input="updateValue('media_allowed_file_types', ($event.target as HTMLInputElement).value)"
        type="text"
        class="w-full px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-input-bg text-admin-theme-text focus:ring-admin-theme-primary focus:border-admin-theme-primary"
      />
      <p class="mt-1 text-sm text-admin-theme-text-muted">
        {{ t('Comma separated list of allowed file extensions for uploads.') }}
      </p>
    </div>

 <div>
 <label for="media_max_upload_size" class="block text-sm font-medium text-admin-theme-text-secondary mb-1">
 ...
 />
 <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">{{ t('Allow customizing the upload directory for media files.') }}</p>
 
 <div v-if="getValue('media_customize_upload_path', false)" class="mt-4 p-4 bg-admin-theme-base/50 rounded-lg border border-admin-theme-border ml-6">
 <label for="media_upload_path" class="block text-sm font-medium text-admin-theme-text-secondary mb-1 flex justify-between">
 ...
 type="text"
 maxlength="250"
 class="w-full px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-input-bg text-admin-theme-text focus:ring-admin-theme-primary focus:border-admin-theme-primary mb-2"
 />
 ...
 />
 <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">{{ t('Enable chunked uploading for very large files.') }}</p>
 </div>
 </div>
 </div>
 ...
 class="mr-2 text-admin-theme-primary focus:ring-admin-theme-primary form-radio disabled:bg-gray-200 dark:disabled:bg-gray-700"
 />
 ...
 <!-- Thumbnails -->
 <div class="pt-4 border-t border-admin-theme-border space-y-4">
 <div>
 <FormCheckbox
 ...
 />
 <p class="mt-1 text-sm text-admin-theme-text-muted pl-6">{{ t('Generate smaller thumbnail versions of uploaded images.') }}</p>
 </div>

  <div v-if="getValue('media_enable_thumbnails', true)" class="p-6 border border-admin-theme-border rounded-lg space-y-4 bg-admin-theme-base/50">
    <div class="flex items-center justify-between">
      <h4 class="font-medium text-admin-theme-text">{{ t('Media thumbnails sizes:') }}</h4>
      <span class="text-xs text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1">
        <i class="fas fa-check-circle"></i>
        {{ t('Proportional aspect ratio preserved') }}
      </span>
    </div>
    <p class="text-xs text-admin-theme-text-muted">
      {{ t('Maximum bounding dimensions. Images will be scaled proportionally to preserve original aspect ratio without square cropping.') }}
    </p>
  
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">Thumb (Max: 150x150)</label>
        <input
      ...
      <div>
        <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">&nbsp;</label>
        <input
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
      <div>
        <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">Featured (Max: 565x375)</label>
        <input
    ...
    <div class="pt-3 border-t border-admin-theme-border flex items-center justify-between flex-wrap gap-2">
      <div class="text-xs text-admin-theme-text-muted">
        {{ t('Re-create WebP thumbnails for all existing media uploaded previously?') }}
      </div>
      <button
        type="button"
        @click="regenerateAllThumbnails"
        :disabled="regenerating"
        class="px-3.5 py-1.5 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors shadow-sm disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
      >
        <i class="fas" :class="regenerating ? 'fa-spinner fa-spin' : 'fa-sync-alt'"></i>
        {{ regenerating ? t('Processing...') : t('Re-generate All Thumbnails') }}
      </button>
    </div>
  </div>
  </div>
 ...
 <!-- Save Button -->
 <div class="flex justify-end pt-4 border-t border-admin-theme-border mt-8">
 <button
 type="submit"
 :disabled="saving"
...
const regenerateAllThumbnails = async () => {
 ...
 title: t('Re-generate All Thumbnails'),
 message: t('Are you sure you want to re-create WebP proportional thumbnails for all uploaded media files?'),
 confirmText: t('Re-generate'),
 cancelText: t('Cancel'),
 type: 'warning'
 });
 ...
 const response = await axios.post('/api/v1/media/regenerate-thumbnails');
 dialog.success(response.data?.message || t('Thumbnails regenerated successfully!'));
 } catch (err: any) {
 dialog.error(err.response?.data?.message || t('Failed to regenerate thumbnails.'));
 } finally {
