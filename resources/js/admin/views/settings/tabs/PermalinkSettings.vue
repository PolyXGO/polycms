<template>
 ...
 <p v-if="section.description" class="mt-1 text-sm text-admin-theme-text-muted" v-html="$t(section.description)"></p>
 </div>
 </div>
 ...
 class="space-y-2"
 >
 <label :for="setting.key" class="block text-sm font-medium text-admin-theme-text-secondary">
 {{ $t(setting.label) }}
 </label>
 <div class="flex h-11 items-stretch overflow-hidden rounded-lg border border-admin-theme-border bg-admin-theme-input-bg focus-within:ring-2 focus-within:ring-admin-theme-primary focus-within:border-admin-theme-primary">
 <span class="flex items-center px-3 text-sm text-admin-theme-text-muted">
 ...
 type="text"
 class="flex-1 border-0 bg-transparent px-3 text-admin-theme-text placeholder-admin-theme-text-muted focus:outline-none focus:ring-0"
 :placeholder="setting.input_props?.placeholder ||''"
 />
 </div>
 <p v-if="setting.description" class="text-sm text-admin-theme-text-muted" v-html="$t(setting.description)"></p>
 ...
 <div class="border-t border-admin-theme-border pt-4">
 <button
 type="submit"
 :disabled="saving"
 class="inline-flex items-center rounded-lg bg-admin-theme-primary px-4 py-2 text-sm font-medium text-admin-theme-primary-content transition-colors hover:bg-admin-theme-primary-hover disabled:cursor-not-allowed disabled:opacity-60"
 >
 <svg
 v-if="saving"
...
const buildBasePrefix = (key: string): string => {
 ...
 case'permalink_projects_archive':
 case'permalink_projects_single':
...
const buildPreview = (key: string): string => {
 ...
 case'permalink_projects_archive':
 return `${baseUrl}${slug ||'/projects'}`;
 case'permalink_projects_single':
 return `${baseUrl}${slug ||'/projects'}/sample-project`;
 case'permalink_category_base':
...
<style scoped>
:deep(a) {
  color: rgb(var(--admin-theme-primary)) !important;
  text-decoration: underline !important;
}
:deep(a:hover) {
  color: rgb(var(--admin-theme-primary-hover)) !important;
}
</style>
