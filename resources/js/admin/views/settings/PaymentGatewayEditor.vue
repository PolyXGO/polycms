<template>
 ...
 <router-link :to="{ name:'admin.settings.gateways' }" class="text-admin-theme-primary hover:text-admin-theme-primary-hover font-medium flex items-center">
 <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
 </svg>
 {{ t('Back to Gateways') }}
 </router-link>
 <h1 class="text-2xl font-bold text-admin-theme-text">{{ t('Configure') }} {{ gateway.name }}</h1>
 </div>
 ...
 <form v-else @submit.prevent="saveConfig" class="bg-admin-theme-surface rounded-lg shadow p-6">
 <div class="space-y-6">
 ...
 rows="8"
 class="w-full px-3 py-2 font-mono text-sm border border-admin-theme-border rounded-lg bg-admin-theme-input-bg text-admin-theme-text"
 ...
 type="checkbox"
 v-model="config[field.key]"
 class="h-4 w-4 text-admin-theme-primary rounded"
 />
 <span class="ml-2 text-sm text-admin-theme-text-secondary">{{ t('Enabled') }}</span>
 </label>
 </div>
 <p v-if="field.description" class="mt-1 text-xs text-admin-theme-text-muted">
 {{ field.description }}
 </p>
 ...
 <div class="pt-6 border-t border-admin-theme-border">
 <button 
 type="submit"
 :disabled="saving"
 class="px-6 py-2 bg-admin-theme-primary text-admin-theme-primary-content rounded-lg hover:bg-admin-theme-primary-hover disabled:opacity-50"
 >
