<template>
 ...
 <div>
 <h1 class="text-2xl font-bold text-admin-theme-text">Theme Options</h1>
 ...
 <button
 type="button"
 class="px-4 py-2 bg-admin-theme-primary text-admin-theme-primary-content rounded-lg hover:bg-admin-theme-primary-hover transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
 ...
 <div v-if="loading" class="bg-admin-theme-surface rounded-lg shadow p-12 text-center">
 <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-admin-theme-primary"></div>
 <p class="mt-3 text-admin-theme-text-secondary text-sm">Loading theme options...</p>
 </div>

 <div v-else-if="sections.length === 0" class="bg-admin-theme-surface rounded-lg shadow p-12 text-center">
 <h2 class="text-lg font-semibold text-admin-theme-text">No Theme Options Registered</h2>
 ...
 :key="section.key"
 class="bg-admin-theme-surface rounded-lg shadow"
 >
 <div class="px-6 py-5 border-b border-admin-theme-border flex items-center justify-between">
 <div class="flex items-center gap-2">
 <h2 class="text-xl font-semibold text-admin-theme-text">{{ section.label }}</h2>
 <HelpGuide
 ...
 class="space-y-4"
 >
 <div v-if="category.label" class="flex items-center gap-2">
 <h3 class="text-sm font-semibold text-admin-theme-text-secondary uppercase tracking-wide">{{ category.label }}</h3>
 <HelpGuide
 ...
 :key="setting.key"
 class="bg-admin-theme-base/30 border border-admin-theme-border rounded-lg p-5 space-y-3"
 >
 <div class="flex items-start justify-between gap-3">
 <div>
 <label class="text-sm font-medium text-admin-theme-text">{{ setting.label }}</label>
 <p v-if="setting.description" class="text-xs text-admin-theme-text-muted mt-1">{{ setting.description }}</p>
 </div>
 <button
 type="button"
 class="text-xs text-admin-theme-primary dark:text-admin-theme-primary hover:underline"
 ...
 class="h-10 w-16 border border-admin-theme-border rounded-lg bg-admin-theme-surface"
 />
 <input
 type="text"
 v-model="setting.value"
 class="flex-1 px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-surface text-admin-theme-text focus:outline-none focus:ring-2 focus:ring-admin-theme-primary/50 shadow-sm"
 />
 </div>
 </template>
 ...
 <select
 v-model="setting.value"
 class="w-full px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-surface text-admin-theme-text focus:outline-none focus:ring-2 focus:ring-admin-theme-primary/50 shadow-sm appearance-none"
 >
 <option
 v-for="option in setting.options"
 :key="option.value"
 :value="option.value"
 class="bg-admin-theme-surface text-admin-theme-text"
 >
 {{ option.label }}
 </option>
 </select>
 ...
 class="w-full px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-surface text-admin-theme-text focus:outline-none focus:ring-2 focus:ring-admin-theme-primary/50 shadow-sm"
 ...
 class="text-sm text-admin-theme-text-muted"
 >
 ...
 <template v-else-if="setting.type === 'html' || setting.type === 'editor'">
 <TiptapEditor
 v-model="setting.value"
 :editable="true"
 :placeholder="setting.description || 'Enter HTML...'"
 />
 </template>
 ...
 <p class="text-xs text-admin-theme-text-muted" v-if="setting.type ==='number' && setting.input_props?.step">
...
const sections = computed<ThemeSectionGroup[]>(() => {
 ...
 description: setting.category_description ?? null,
