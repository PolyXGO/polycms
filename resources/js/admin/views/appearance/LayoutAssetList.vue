<template>
 ...
 <h1 class="mt-1 text-2xl font-bold text-admin-theme-text">{{ kindLabel }}</h1>
 ...
 <div class="rounded-lg border border-admin-theme-border bg-admin-theme-surface p-4 shadow">
 <div class="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-admin-theme-text-muted">Filters</div>
 <div class="space-y-2">
 <button
 type="button"
 ...
 :'text-admin-theme-text-secondary hover:bg-black/5 dark:hover:bg-white/5'"
 ...
 :'bg-admin-theme-base text-admin-theme-text-secondary'"
 >
 {{ totalCount }}
 </span>
 </button>
 ...
 <div class="rounded-lg border border-admin-theme-border bg-admin-theme-surface p-4 shadow">
 <div class="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-admin-theme-text-muted">Source</div>
 ...
 ?'border-admin-theme-primary bg-admin-theme-primary/10 text-admin-theme-primary dark:border-admin-theme-primary dark:bg-admin-theme-primary/100/10'
 :'bg-admin-theme-input-bg text-admin-theme-text-secondary hover:bg-black/5 dark:hover:bg-admin-theme-input-bg/5 border-admin-theme-input-border'"
 ...
 class="w-full rounded-lg border bg-admin-theme-input-bg py-2.5 pl-11 pr-4 text-sm text-admin-theme-text placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-admin-theme-primary border-admin-theme-input-border dark:placeholder-gray-500"
 @input="handleSearch"
 />
 </label>

 <div class="inline-flex overflow-hidden rounded-lg border border-admin-theme-border">
 <button
 type="button"
 class="inline-flex h-11 w-11 items-center justify-center transition-colors"
 :class="viewMode ==='grid'
 ?'bg-admin-theme-primary text-admin-theme-primary-content'
 :'bg-admin-theme-surface text-admin-theme-text-secondary hover:bg-admin-theme-base'"
 @click="viewMode ='grid'"
 >
 ...
 <button
 type="button"
 class="inline-flex h-11 w-11 items-center justify-center border-l border-admin-theme-border transition-colors"
 :class="viewMode ==='list'
 ?'bg-admin-theme-primary text-admin-theme-primary-content'
 :'bg-admin-theme-surface text-admin-theme-text-secondary hover:bg-admin-theme-base'"
 @click="viewMode ='list'"
 ...
 <p class="text-sm text-admin-theme-text-secondary">Loading {{ kindLabel.toLowerCase() }}...</p>
 </div>

 <div v-else-if="assets.length === 0" class="p-12 text-center">
 <div class="text-lg font-semibold text-admin-theme-text">No {{ kindLabel.toLowerCase() }} found</div>
 ...
 :key="asset.id"
 class="overflow-hidden rounded-lg border border-admin-theme-border bg-admin-theme-base"
 >
 <div
 class="relative aspect-[16/10] w-full cursor-pointer overflow-hidden border-b border-admin-theme-border bg-admin-theme-base text-left"
 role="button"
 tabindex="0"
 ...
 :alt="asset.name"
 class="absolute left-0 top-0 h-full w-full object-contain object-top transition-opacity duration-300"
 loading="lazy"
 />
 ...
 @thumbnail-generated="(url) => handleThumbnailGenerated(asset, url)"
 />
 <span
 v-if="asset.is_system"
 class="absolute left-3 top-3 inline-flex rounded-full bg-admin-theme-surface/90 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-admin-theme-text-secondary shadow-sm"
 >
 Default
 </span>
 </div>
 ...
 <div class="min-w-0">
 <h3 class="text-sm font-semibold text-admin-theme-text">{{ asset.name }}</h3>
 <p class="mt-1 break-all text-xs text-admin-theme-text-muted">/{{ asset.slug }}</p>
 </div>
 <span class="flex shrink-0 items-center gap-1 rounded-full bg-gray-200 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-admin-theme-text-secondary dark:bg-white/10">
 <component :is="getCategoryIcon(asset.category || 'general')" class="h-3 w-3" />
 {{ getCategoryLabel(asset.category || 'general') }}
 </span>
 </div>

 <p class="text-sm leading-6 text-admin-theme-text-secondary">
 ...
 :key="applyTarget"
 class="rounded-full bg-admin-theme-primary/10 px-2.5 py-1 text-[11px] font-semibold text-admin-theme-primary dark:bg-admin-theme-primary/100/10"
 >
 {{ applyTarget }}
 </span>
 </div>

 <div class="flex flex-wrap gap-3 border-t border-admin-theme-border pt-3">
 <button
 type="button"
 class="text-sm font-semibold text-gray-600 transition-colors hover:text-admin-theme-text"
 title="Regenerate Thumbnail"
 @click="regenerateThumbnail(asset)"
 >
 Regenerate
 </button>
 <button
 type="button"
 class="text-sm font-semibold text-admin-theme-primary transition-colors hover:text-admin-theme-primary dark:hover:text-admin-theme-primary"
 @click="openAsset(asset)"
 >
 ...
 <div v-else class="divide-y divide-admin-theme-border">
 <article
 v-for="asset in assets"
 :key="asset.id"
 ...
 >
 <div
 class="relative h-24 w-full shrink-0 cursor-pointer overflow-hidden rounded-lg border border-admin-theme-border bg-admin-theme-base text-left xl:w-48"
 role="button"
 tabindex="0"
 @click="openAsset(asset)"
 @keydown.enter.prevent="openAsset(asset)"
 ...
 <div class="flex flex-wrap gap-2">
 <span class="rounded-full bg-gray-200 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-admin-theme-text-secondary">
 {{ asset.category ||'general' }}
 </span>
 <span
 v-if="asset.is_system"
 class="rounded-full bg-admin-theme-primary/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-admin-theme-primary dark:bg-admin-theme-primary/100/10"
 >
 Default
 </span>
 </div>
 ...
 class="flex items-center justify-between border-t border-admin-theme-border px-5 py-4"
 >
 <button
 type="button"
 class="rounded-lg border bg-admin-theme-input-bg px-4 py-2 text-sm font-medium text-admin-theme-text-secondary transition-colors hover:bg-black/5 dark:hover:bg-admin-theme-input-bg/5 disabled:cursor-not-allowed disabled:opacity-50 border-admin-theme-input-border"
 :disabled="meta.current_page <= 1"
 @click="changePage(meta.current_page - 1)"
 >
 Previous
 </button>
 <span class="text-sm text-admin-theme-text-muted">Page {{ meta.current_page }} of {{ meta.last_page }}</span>
 <button
 type="button"
 class="rounded-lg border bg-admin-theme-input-bg px-4 py-2 text-sm font-medium text-admin-theme-text-secondary transition-colors hover:bg-black/5 dark:hover:bg-admin-theme-input-bg/5 disabled:cursor-not-allowed disabled:opacity-50 border-admin-theme-input-border"
...
const categoryMeta: Record<string, { label: string; icon: Component }> = {
 ...
 pricing: { label: 'Pricing', icon: CurrencyDollarIcon },
 content: { label: 'Content', icon: DocumentTextIcon },
 ecommerce: { label: 'E-Commerce', icon: ShoppingCartIcon },
 contact: { label: 'Contact', icon: EnvelopeIcon },
 'social-proof': { label: 'Social Proof', icon: TrophyIcon },
 navigation: { label: 'Navigation', icon: Bars3Icon },
 media: { label: 'Media', icon: FilmIcon },
 default: { label: 'Default', icon: CubeIcon },
 general: { label: 'General', icon: CubeIcon },
 landing: { label: 'Landing Page', icon: RocketLaunchIcon },
 blog: { label: 'Blog', icon: PencilSquareIcon },
 product: { label: 'Product', icon: TagIcon },
 portfolio: { label: 'Portfolio', icon: BriefcaseIcon },
 documentation: { label: 'Documentation', icon: BookOpenIcon },
 'coming-soon': { label: 'Coming Soon', icon: ClockIcon },
};

const dbCategories = ref<Record<string, { label: string }>>({});
...
const loadCategories = async () => {
  try {
    ...
    for (const cat of categoriesData) {
      map[cat.slug] = { label: cat.name };
    }
    dbCategories.value = map;
  } catch (error) {
    console.error('Failed to load categories', error);
  }
};

const getCategoryLabel = (key: string) => dbCategories.value[key]?.label || categoryMeta[key]?.label || key.replace(/[-_]/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
const getCategoryIcon = (key: string) => categoryMeta[key]?.icon || CubeIcon;

const handleThumbnailGenerated = (asset: LayoutAssetSummary, url: string) => {
 asset.preview_image = url;
};

const regenerateThumbnail = (asset: LayoutAssetSummary) => {
 asset.preview_image = null;
};

const categoryAggregations = ref<Record<string, number>>({});

const categories = computed(() => {
 return Object.entries(categoryAggregations.value)
 .map(([name, count]) => {
 const catName = (!name || name === 'null') ? 'general' : name;
 return {
 name: catName,
...
const fetchAssets = async (page = 1) => {
 ...
 categoryAggregations.value = response.data?.aggregations?.categories || {};
 meta.value = {
