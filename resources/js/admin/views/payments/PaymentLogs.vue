<template>
 ...
 <h1 class="text-2xl font-bold text-admin-theme-text">{{ t('Payment Logs') }}</h1>
 ...
 <button 
 @click="() => loadLogs()"
 class="px-4 py-2 text-sm bg-admin-theme-base rounded-lg hover:bg-admin-theme-base"
 >
 ...
 v-for="log in logs" 
 :key="log.id"
 class="p-4 hover:bg-admin-theme-base"
 >
 ...
 <span class="text-sm font-medium text-admin-theme-text">{{ log.gateway }}</span>
 <span class="text-xs px-2 py-0.5 rounded capitalize" :class="getLevelBadgeClass(log.level)">
 {{ log.level }}
 </span>
 </div>
 <p class="text-sm text-admin-theme-text-secondary">{{ log.message }}</p>
 <p class="text-xs text-admin-theme-text-muted mt-1">{{ formatDate(log.created_at) }}</p>
 </div>
 </div>
 <button 
 v-if="log.context"
 @click="toggleExpand(log)"
 class="text-xs text-admin-theme-primary hover:text-admin-theme-primary"
 >
 ...
 <pre class="bg-admin-theme-base p-3 rounded text-xs overflow-x-auto">{{ JSON.stringify(log.context, null, 2) }}</pre>
 </div>
 </div>
 
 <div v-if="logs.length === 0" class="p-12 text-center text-admin-theme-text-muted">
