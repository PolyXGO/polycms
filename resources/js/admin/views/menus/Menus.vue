<template>
 ...
 <div>
 <h1 class="text-2xl font-bold text-admin-theme-text">{{ $t('Menus') }}</h1>
 ...
 class="px-4 py-2 border border-red-300 dark:border-red-600 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 bg-admin-theme-input-bg text-red-600 dark:text-red-400 transition-colors"
 >
 {{ $t('Delete Menu') }}
 </button>
 <button
 ...
 <!-- Menu Selector -->
 <div v-if="menus.length > 0" class="bg-admin-theme-surface rounded-lg shadow p-4">
 ...
 <!-- Empty State -->
 <div v-else-if="!loading && menus.length === 0" class="bg-admin-theme-surface rounded-lg shadow p-12 text-center">
 <svg class="mx-auto h-12 w-12 text-admin-theme-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
 </svg>
 <h3 class="mt-2 text-sm font-medium text-admin-theme-text">{{ $t('No menus yet') }}</h3>
 ...
 <p class="mt-2 text-admin-theme-text-secondary">{{ $t('Loading menus...') }}</p>
 </div>
 ...
 >
 <div class="bg-admin-theme-surface rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
 <h3 class="text-lg font-semibold text-admin-theme-text mb-4">
 {{ $t('Create New Menu') }}
 </h3>
 <div class="mb-6">
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-2">
 ...
 class="w-full px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-input-bg text-admin-theme-text focus:outline-none focus:ring-2 focus:ring-admin-theme-primary focus:border-admin-theme-primary transition-colors"
 ...
 class="px-4 py-2 bg-admin-theme-primary text-admin-theme-primary-content rounded-lg hover:bg-admin-theme-primary-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {{ $t('Create') }}
 </button>
 </div>
 ...
 >
 <div class="bg-admin-theme-surface rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
 <h3 class="text-lg font-semibold text-admin-theme-text mb-2">
 {{ $t('Delete Menu') }}
 </h3>
 <p class="text-sm text-admin-theme-text-secondary mb-6">
...
const loadMenus = async () => {
 ...
 // Auto-select 'main' menu if available, otherwise first menu
 if (menus.value.length > 0 && !selectedMenuId.value) {
 const mainMenu = menus.value.find(m => m.slug === 'main' || m.name.toLowerCase() === 'main');
 selectedMenuId.value = mainMenu ? mainMenu.id : menus.value[0].id;
 await loadMenu();
 }
 } catch (error: any) {
