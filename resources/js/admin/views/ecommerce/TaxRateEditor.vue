<template>
 ...
 <router-link :to="{ name:'admin.ecommerce.tax-rates.index' }" class="text-admin-theme-text-muted hover:text-gray-700 dark:text-admin-theme-text-muted mr-4">
 ...
 <!-- Name -->
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Tax Name') }} *</label>
 <input v-model="form.name" type="text" required :class="{'border-red-500': errors.name}" class="w-full px-3 py-2 border border-admin-theme-border rounded-md bg-admin-theme-input-bg text-admin-theme-text" placeholder="e.g. VAT, State Tax" />
 <p v-if="errors.name" class="mt-1 text-sm text-red-500">{{ errors.name[0] }}</p>
 </div>

 <!-- Location -->
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Location (Country / State)') }} *</label>
 ...
 <!-- Rate -->
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Tax Rate') }} (%) *</label>
 <input v-model="displayRate" @input="updateRate" type="number" step="0.01" min="0" required class="w-full px-3 py-2 border border-admin-theme-border rounded-md bg-admin-theme-input-bg text-admin-theme-text" placeholder="e.g. 10" />
 ...
 <input id="is_compound" v-model="form.is_compound" type="checkbox" class="h-4 w-4 text-admin-theme-primary focus:ring-admin-theme-primary border-gray-300 rounded" />
 <label for="is_compound" class="ml-2 block text-sm text-admin-theme-text">
 {{ $t('Compound Tax') }}
