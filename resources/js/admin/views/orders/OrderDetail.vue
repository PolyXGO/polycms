    ...
    <div v-if="item.metadata?.license_key" class="flex flex-col gap-1 mt-1">
      <div class="flex items-center gap-2">
        ...
        <button 
            ...
            type="button" 
            class="text-admin-theme-text-muted hover:text-admin-theme-text transition-colors p-0.5 cursor-pointer flex items-center justify-center"
            :title="revealedKeys[item.id] ? $t('Hide Key') : $t('Show Key')"
        >
            <svg v-if="!revealedKeys[item.id]" xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
