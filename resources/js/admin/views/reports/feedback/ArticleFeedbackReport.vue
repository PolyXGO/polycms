<template>
 ...
 <div>
 <router-link :to="{ name:'admin.reports.index' }" class="text-sm text-admin-theme-text-muted hover:text-admin-theme-primary dark:hover:text-admin-theme-primary mb-1 inline-flex items-center gap-1">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
 Reports
 </router-link>
 <h1 class="text-2xl font-bold text-admin-theme-text">Article Feedback</h1>
 </div>
 <div class="flex gap-2">
 <select v-model="dateRange" @change="fetchData" class="text-sm border border-admin-theme-border rounded-lg px-3 py-2 bg-admin-theme-input-bg text-admin-theme-text">
 ...
 <div class="bg-admin-theme-surface rounded-xl border border-admin-theme-border p-5">
 <p class="text-sm font-medium text-admin-theme-text-muted">Total Votes</p>
 <p class="text-3xl font-bold text-admin-theme-text mt-1">{{ stats.total }}</p>
 </div>
 <div class="bg-admin-theme-surface rounded-xl border border-admin-theme-border p-5">
 <p class="text-sm font-medium text-admin-theme-text-muted">Helpful</p>
 ...
 <div class="bg-admin-theme-surface rounded-xl border border-admin-theme-border p-5">
 <p class="text-sm font-medium text-admin-theme-text-muted">Not Helpful</p>
 ...
 <!-- Chart -->
 <div class="bg-admin-theme-surface rounded-xl border border-admin-theme-border p-6 mb-8">
 <h2 class="text-lg font-semibold text-admin-theme-text mb-4">Daily Trend</h2>
 <canvas ref="chartCanvas" height="100"></canvas>
 </div>

 <!-- Votes Table -->
 <div class="bg-admin-theme-surface rounded-xl border border-admin-theme-border overflow-hidden">
 <div class="px-6 py-4 border-b border-admin-theme-border flex items-center justify-between">
 <h2 class="text-lg font-semibold text-admin-theme-text">Recent Votes</h2>
 <select v-model="filterType" @change="() => fetchVotes()" class="text-sm border border-admin-theme-border rounded-lg px-3 py-1.5 bg-admin-theme-input-bg text-admin-theme-text">
 ...
 <thead>
 <tr class="bg-admin-theme-base">
 <th class="text-left px-6 py-3 font-medium text-admin-theme-text-muted">Article</th>
 <th class="text-left px-6 py-3 font-medium text-admin-theme-text-muted">Vote</th>
 <th class="text-left px-6 py-3 font-medium text-admin-theme-text-muted">IP</th>
 <th class="text-left px-6 py-3 font-medium text-admin-theme-text-muted">Date</th>
 </tr>
 </thead>
 <tbody class="divide-y divide-admin-theme-border">
 <tr v-for="vote in votes" :key="vote.id" class="hover:bg-admin-theme-base/30 transition-colors">
 <td class="px-6 py-3">
 <a v-if="vote.content_url" :href="vote.content_url" target="_blank" class="text-admin-theme-primary dark:text-admin-theme-primary hover:underline">
 {{ vote.content_title || `#${vote.voteable_id}` }}
 </a>
 <span v-else class="text-admin-theme-text-secondary">{{ vote.content_title || `#${vote.voteable_id}` }}</span>
 </td>
 <td class="px-6 py-3">
 ...
 <td class="px-6 py-3 text-admin-theme-text-muted font-mono text-xs">{{ vote.ip_address }}</td>
 <td class="px-6 py-3 text-admin-theme-text-muted">{{ formatDate(vote.created_at) }}</td>
 </tr>
 <tr v-if="votes.length === 0">
 <td colspan="4" class="px-6 py-8 text-center text-admin-theme-text-muted">No votes recorded yet.</td>
 </tr>
 </tbody>
 </table>
 </div>
 <!-- Pagination -->
 <div v-if="pagination.lastPage > 1" class="px-6 py-3 border-t border-admin-theme-border flex justify-between items-center">
 ...
 class="px-3 py-1 text-sm border border-admin-theme-border rounded-lg disabled:opacity-40 hover:bg-admin-theme-base text-admin-theme-text-secondary">
 Previous
 </button>
