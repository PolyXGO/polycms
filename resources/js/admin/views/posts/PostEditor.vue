<template>
 ...
 Cancel
  ...
  <AdminLoadingState
    v-if="isFetchingData"
    :message="panelType === 'page' ? ($t('Loading page data...') || 'Đang tải dữ liệu trang...') : ($t('Loading post data...') || 'Đang tải dữ liệu bài viết...')"
  />
  <form v-else class="editor-form" @submit.prevent="savePost">
...
const getPermalink = (): string => {
 ...
 const baseUrl = window.location.origin;
  ...
  if (isFlexidocsTemplate.value && form.value.type !== 'page') {
 ...
 return buildUrl(type, form.value.slug, form.value.locale);
};
...
const loadPost = async () => {
  ...
  if (route.query.lang && route.query.lang !== post.locale) {
    const targetLang = route.query.lang;
    const translation = (post.translations || []).find((t: any) => t.locale === targetLang);
    if (translation) {
      const editRouteName = post.type === 'page' ? 'admin.pages.edit' : 'admin.posts.edit';
 ...
 show_title: post.show_title ?? true,
...
const savePost = async () => {
 ...
 show_title: form.value.show_title ?? true,
...
useGlobalSaveHotkey(() => savePost(), computed(() => loading.value || isFetchingData.value));
</script>
