<template>
 ...
 {{ isEditing ? ($t('Edit Product Brand') ||'Edit Product Brand') : ($t('New Brand') ||'New Brand') }}
 </h1>
 ...
 <div class="space-y-4">
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Parent Brand') ||'Parent Brand' }}</label>
 <select
 ...
 <!-- Brand Logo -->
 <div class="bg-admin-theme-surface rounded-lg shadow p-6">
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-3">{{ $t('Brand Logo') ||'Brand Logo' }}</label>
 <div
 @click="openMediaLibrary"
 class="relative aspect-square rounded-lg border-2 border-dashed border-admin-theme-border flex flex-col items-center justify-center cursor-pointer hover:border-admin-theme-primary transition-colors overflow-hidden group"
 >
 <template v-if="form.image">
 <img :src="form.image" class="w-full h-full object-contain p-4" />
 <div class="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
 <span class="text-white text-sm font-medium">{{ $t('Change Logo') ||'Change Logo' }}</span>
 </div>
 <button
 ...
 <span class="text-sm text-admin-theme-text-muted">{{ $t('Choose logo') ||'Choose logo' }}</span>
 </template>
 </div>
 </div>
 </div>
 ...
 :disabled="saving" 
 @click="save"
 :title="saving ? ($t('Saving…') ||'Saving…') : ($t('Save Brand') ||'Save Brand')"
 >
...
const slugPrefix = computed(() => {
 ...
 const full = getProductBrandUrl(form.value.slug);
...
const fullPermalink = computed(() => {
 return getProductBrandUrl(form.value.slug);
});
...
const availableParents = computed(() => {
 return allBrands.value.filter(b => b.id !== Number(route.params.id));
});
...
const loadBrand = async () => {
  ...
  try {
    const response = await axios.get(`/api/v1/product-brands/${route.params.id}`);
    const brand = response.data.data;

    // Transition to translation if requested via ?lang= locale query parameter
    if (route.query.lang && route.query.lang !== brand.locale) {
      const targetLang = route.query.lang;
      const translation = (brand.translations || []).find((t: any) => t.locale === targetLang);
      if (translation) {
        const editRouteName = 'admin.product-brands.edit';
    ...
    dialog.error('Failed to load brand');
  }
};
...
const save = async () => {
 ...
 try {
 if (isEditing.value) {
 await axios.put(`/api/v1/product-brands/${route.params.id}`, form.value);
 await loadBrand();
 dialog.success('Brand updated successfully');
 } else {
 const response = await axios.post('/api/v1/product-brands', form.value);
 dialog.success('Brand created successfully');
 router.push({
...
onMounted(async () => {
 landingStore.setPostType('product-brands');
