<template>
 ...
 <div>
 <h1 class="text-2xl font-bold text-admin-theme-text">{{ $t('Product Attributes') }}</h1>
 ...
 class="inline-flex items-center px-4 py-2 border border-admin-theme-border shadow-sm text-sm font-medium rounded-md text-admin-theme-text bg-admin-theme-base hover:bg-admin-theme-base transition-colors"
 >
 <svg class="-ml-1 mr-2 h-5 w-5 text-admin-theme-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
 {{ $t('Manage Groups') }}
 </router-link>
 <button
 ...
 <AdminLoadingState v-if="loading" :message="$t('Loading product attributes...')" />

 <div v-else-if="attributes.length === 0" class="text-center py-16 bg-admin-theme-surface rounded-lg border border-admin-theme-border shadow-sm">
 <svg class="mx-auto h-12 w-12 text-admin-theme-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
 <h3 class="mt-4 text-sm font-medium text-admin-theme-text">{{ $t('No attributes') }}</h3>
 <p class="mt-1 text-sm text-admin-theme-text-secondary">{{ $t('Get started by creating a new attribute like Size or Color.') }}</p>
 <div class="mt-6">
 <button @click="openAttributeModal()" type="button" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-admin-theme-primary-content bg-admin-theme-primary hover:bg-admin-theme-primary-hover">
 ...
 <div>
 <h2 class="text-lg font-semibold text-admin-theme-text-secondary mb-3 ml-1">
 ...
 :key="attr.id"
 class="bg-admin-theme-surface rounded-lg border border-admin-theme-border shadow-sm overflow-hidden"
 >
 <div class="px-4 py-4 sm:px-6 flex items-center justify-between bg-admin-theme-base/50 border-b border-admin-theme-border">
 <div class="flex items-center gap-3">
 <h3 class="text-lg font-medium text-admin-theme-text">{{ attr.name }}</h3>
 <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-admin-theme-input-bg text-admin-theme-text">
 {{ formatDisplayType(attr.display_type) }}
 </span>
 <span class="text-xs text-admin-theme-text-secondary">Slug: {{ attr.slug }}</span>
 </div>
 <div class="flex items-center gap-2">
 <button @click="openAttributeModal(attr)" class="text-admin-theme-text-muted hover:text-admin-theme-primary transition-colors" :title="$t('Edit Attribute')">
 ...
 <button @click="deleteAttribute(attr.id)" class="text-admin-theme-text-muted hover:text-red-500 transition-colors" :title="$t('Delete Attribute')">
 ...
 :key="val.id"
 class="group relative inline-flex items-center gap-1.5 pl-2.5 pr-2 py-1 bg-admin-theme-input-bg border border-admin-theme-border rounded-md text-sm font-medium cursor-pointer hover:border-indigo-400 hover:border-admin-theme-primary transition-colors"
 ...
 ></span>
 <span class="text-admin-theme-text">{{ val.value }}</span>
 <!-- Delete Value button (visible on hover) -->
 <button
 @click.stop="deleteValue(attr.id, val.id)"
 class="opacity-0 group-hover:opacity-100 ml-1 text-admin-theme-text-muted hover:text-red-500 transition-all p-0.5"
 ...
 class="w-28 px-2 py-1 min-h-[30px] rounded-md border-admin-theme-border border-dashed dark:bg-admin-theme-base text-sm focus:ring-admin-theme-primary focus:border-admin-theme-primary placeholder-gray-400 transition-all focus:w-40"
 @keydown.enter.prevent="quickAddValue(attr.id)"
 />
 <!-- Fast color picker for Color attribute -->
 <div v-if="attr.display_type ==='color_swatch'" class="absolute right-1 top-1 w-6 h-6 overflow-hidden rounded cursor-pointer border border-admin-theme-border">
 <input
 ...
 <div class="inline-block align-bottom bg-admin-theme-surface rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
 ...
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Attribute Group') }}</label>
 <input
 ...
 <p class="mt-1 text-xs text-admin-theme-text-secondary">{{ $t('Optional. Groups help categorize technical items like SSD/RAM into Hardware.') }}</p>
 </div>
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Attribute Name') }} <span class="text-red-500">*</span></label>
 <input
 ...
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Slug') }}</label>
 <input
 v-model="attributeForm.slug"
 type="text"
 class="w-full px-3 py-2 border border-admin-theme-border rounded-md shadow-sm focus:ring-admin-theme-primary focus:border-admin-theme-primary sm:text-sm font-mono text-xs"
 />
 <p class="mt-1 text-xs text-admin-theme-text-secondary">{{ $t('Used for URLs and system references. Leave empty to auto-generate.') }}</p>
 </div>
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Display Type') }}</label>
 <select
 ...
 <div class="bg-admin-theme-base/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-admin-theme-border">
 <button
 type="button"
 @click="saveAttribute"
 :disabled="isSaving"
 class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-admin-theme-primary text-base font-medium text-admin-theme-primary-content hover:bg-admin-theme-primary-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-admin-theme-primary sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50"
 >
 {{ $t('Save') }}
 </button>
 <button
 ...
 class="mt-3 w-full inline-flex justify-center rounded-md border border-admin-theme-border shadow-sm px-4 py-2 bg-admin-theme-input-bg text-base font-medium text-admin-theme-text hover:bg-admin-theme-bg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-admin-theme-primary sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
 >
 {{ $t('Cancel') }}
 </button>
 </div>
 ...
 <div>
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Value Label') }} <span class="text-red-500">*</span></label>
 <input
 ...
 <label class="block text-sm font-medium text-admin-theme-text-secondary mb-1">{{ $t('Color Hex') }}</label>
 <div class="flex items-center gap-3">
 <input
 v-model="valueForm.hex_color"
 type="color"
 class="h-9 w-16 p-1 border border-admin-theme-border rounded cursor-pointer"
 />
 <input
 v-model="valueForm.hex_color"
 type="text"
 placeholder="#000000"
 class="flex-1 px-3 py-2 border border-admin-theme-border rounded-md shadow-sm focus:ring-admin-theme-primary focus:border-admin-theme-primary sm:text-sm font-mono uppercase"
 />
 </div>
 </div>
 </div>
