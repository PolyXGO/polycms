<template>
 ...
 {{ $t('Cancel') }}
  ...
  <div v-if="isFetchingData" class="flex flex-col items-center justify-center py-24 bg-admin-theme-surface rounded-xl border border-admin-theme-border shadow-sm">
    <svg class="w-8 h-8 animate-spin text-admin-theme-primary mb-3" fill="none" viewBox="0 0 24 24">
      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
    </svg>
    <span class="text-sm font-medium text-admin-theme-text-secondary">{{ $t('Loading product data...') || 'Đang tải dữ liệu sản phẩm...' }}</span>
  </div>
  <form v-else class="editor-form" @submit.prevent="saveProduct">
...
const panelComponents = {
 ...
 languages: LanguageSelectorPanel,
};
...
const getPermalink = (): string => {
 ...
 return buildUrl('product', form.value.slug, form.value.locale);
};
...
const loadProduct = async () => {
  ...
  if (route.query.lang && route.query.lang !== product.locale) {
    const targetLang = route.query.lang;
    const translation = (product.translations || []).find((t: any) => t.locale === targetLang);
    if (translation) {
      const editRouteName = 'admin.products.edit';
 ...
 translations: product.translations || [],
  ...
  if (!Array.isArray(form.value.settings.purchase_options.buttons)) {
    form.value.settings.purchase_options.buttons = [];
  }

  // Ensure exactly one direct button exists in the list
  const directButtons = form.value.settings.purchase_options.buttons.filter((btn: any) => btn.type === 'direct');
  const externalButtons = form.value.settings.purchase_options.buttons.filter((btn: any) => btn.type === 'external');
  ...
  if (directButtons.length === 0) {
    ...
    // Keep only the first direct button and remove duplicates, preserving order
    let hasFoundDirect = false;
    form.value.settings.purchase_options.buttons = form.value.settings.purchase_options.buttons.filter((btn: any) => {
      if (btn.type === 'direct') {
        if (hasFoundDirect) return false;
        hasFoundDirect = true;
      }
      return true;
    });
...
const saveProduct = async () => {
  if (isFetchingData.value || loading.value) return;
  loading.value = true;
...
useGlobalSaveHotkey(() => saveProduct(), computed(() => loading.value || isFetchingData.value));
</script>
