<template>
 ...
 <form class="bg-admin-theme-surface rounded-lg shadow p-6 space-y-6" @submit.prevent="handleSubmit">
 ...
 :key="role"
 class="flex items-center space-x-3 px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-base"
 >
 <input
 v-model="form.roles"
 type="checkbox"
 ...
 />
 <span class="text-sm font-medium text-admin-theme-text-secondary">{{ role }}</span>
 </label>
 </div>
 <p v-if="form.roles.length === 0" class="mt-2 text-xs text-admin-theme-text-muted">
 ...
 <button
 type="button"
 class="px-4 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-surface text-admin-theme-text-secondary hover:bg-admin-theme-base"
 @click="router.back()"
 >
 {{ t('Cancel') ||'Cancel' }}
 </button>
