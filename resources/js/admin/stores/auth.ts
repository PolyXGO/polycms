...
export const useAuthStore = defineStore('auth', {
    ...
    actions: {
        can(permission: string | string[], requireAll: boolean = false): boolean {
            ...
            if (this.user?.roles?.includes('admin') || this.user?.roles?.includes('super-admin')) {
                return true;
            }
            ...
            return requireAll
                ? permissions.every((item) => userPermissions.includes(item) || userPermissions.includes(normalize(item)))
                : permissions.some((item) => userPermissions.includes(item) || userPermissions.includes(normalize(item)));
        },
        ...
        async checkAuth() {
            if (this.token) {
                ...
                try {
                    axios.defaults.headers.common['Authorization'] = `Bearer ${tokenAtStart}`;
