<template>
 <div class="min-h-screen bg-admin-theme-base flex">
 <!-- Topbar Menu -->
 <TopbarMenu />
 ...
 <aside 
 class="bg-admin-theme-sidebar text-admin-theme-sidebar-text pt-14 pb-6 px-2 fixed left-0 bottom-0 top-0 transition-all duration-300 ease-in-out z-[51] flex flex-col overflow-visible" 
 :class="[
 ...
 <span v-else-if="brandSettingsLoaded && !brandLogo && !(isCollapsed && !isMobile)" class="text-admin-theme-sidebar-text-active text-xl font-semibold uppercase tracking-wide">
 {{ brandName }}
 </span>
 ...
 v-if="!isMobile"
 @click="toggleSidebar"
 class="absolute -right-2 top-1/2 transform translate-x-1/2 -translate-y-1/2 bg-admin-theme-sidebar-hover text-admin-theme-sidebar-text hover:text-admin-theme-sidebar-text-active rounded-full p-1 shadow-md border-2 border-admin-theme-border focus:outline-none z-[9999] w-6 h-6 flex items-center justify-center transition-transform hover:scale-110"
 ...
 <div class="h-5 bg-admin-theme-sidebar-hover rounded animate-pulse"></div>
 </div>
 </div>

 <!-- Profile -->
 <div class="mt-4 border-t border-admin-theme-sidebar-hover pt-4">
 <router-link
 :to="{ name:'admin.profile' }"
 class="flex items-center py-2.5 rounded transition duration-200 text-admin-theme-sidebar-text hover:bg-admin-theme-sidebar-hover hover:text-admin-theme-sidebar-text-active"
 ...
 <button
 @click="toggleSidebar"
 class="w-full flex items-center py-2.5 rounded transition duration-200 text-admin-theme-sidebar-text hover:bg-admin-theme-sidebar-hover hover:text-admin-theme-sidebar-text-active focus:outline-none"
...
onMounted(() => {
  ...
  window.addEventListener('admin-menu-badge-update', handleBadgeUpdate);
...
const handleBadgeUpdate = (e: Event) => {
  ...
  const { key, change } = customEvent.detail || {};
  const item = menuItems.value.find(i => i.key === key);
  if (item) {
    const current = typeof item.badge === 'number' 
      ? item.badge 
      : parseInt(String(item.badge || '0'), 10);
    const next = current + change;
    item.badge = next > 0 ? next : null;
  }
};

onUnmounted(() => {
  window.removeEventListener('admin-menu-changed', handleMenuChanged);
  window.removeEventListener('admin-menu-badge-update', handleBadgeUpdate);
