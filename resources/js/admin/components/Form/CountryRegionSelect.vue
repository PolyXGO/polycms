<template>
 ...
 <div v-if="multiple" class="min-h-[42px] p-2 border rounded-md bg-admin-theme-input-bg flex flex-wrap gap-2 items-center cursor-text" :class="error ?'border-red-500' :'border-admin-theme-border'" @click="openDropdown">
 <template v-for="(item, index) in localValue" :key="index">
 <span class="inline-flex items-center bg-admin-theme-primary/15 text-admin-theme-primary-hover/50 rounded px-2 py-1 text-sm">
 {{ getDisplayName(item) }}
 <button type="button" @click.stop="removeItem(index)" class="ml-1 text-admin-theme-primary hover:text-admin-theme-primary focus:outline-none">
 ...
 v-model="search"
 type="text"
 class="flex-1 outline-none min-w-[120px] bg-transparent text-admin-theme-text"
 ...
 <!-- Closed state -->
 <button v-if="!isOpen" type="button" @click.stop="openDropdown" class="w-full text-left px-3 py-2 border rounded-md bg-admin-theme-input-bg focus:outline-none flex items-center justify-between min-h-[42px]" :class="error ?'border-red-500' :'border-admin-theme-border'">
 <span class="block text-sm truncate" :class="localValue && (localValue as any).country ?'text-admin-theme-text' :'text-gray-400'">
 ...
 v-model="search"
 type="text"
 class="w-full px-3 py-2 border rounded-md bg-admin-theme-input-bg text-admin-theme-text outline-none min-h-[42px]"
 :class="error ?'border-red-500' :'border-admin-theme-primary ring-1 ring-indigo-500'"
 ...
 <!-- Dropdown -->
 <div v-if="isOpen" class="absolute z-50 mt-1 w-full max-h-60 overflow-auto bg-admin-theme-surface border border-admin-theme-border rounded-md shadow-lg py-1">
 ...
 class="px-4 py-2 hover:bg-admin-theme-base cursor-pointer flex items-center"
 :class="{'pl-8 text-sm text-admin-theme-text-secondary': !item.isCountry,'font-medium text-admin-theme-text': item.isCountry }"
 @click="selectItem(item)"
 >
 <div v-if="multiple" class="mr-3 flex-shrink-0">
 <input type="checkbox" :checked="isSelected(item)" class="h-4 w-4 text-admin-theme-primary border-gray-300 rounded pointer-events-none" />
 </div>
