<template>
 ...
 <label v-if="label" class="block text-sm font-medium text-admin-theme-text-secondary mb-1">
 {{ label }}
 </label>
 <select
 v-model="computedValue"
 :disabled="loading"
 class="block w-full px-3 py-2 border border-admin-theme-border rounded-lg bg-admin-theme-input-bg text-admin-theme-text text-sm focus:outline-none focus:ring-1 focus:ring-admin-theme-primary focus:border-admin-theme-primary disabled:opacity-50"
 >
 ...
 <p v-if="help" class="mt-1 text-xs text-admin-theme-text-muted">{{ help }}</p>
 <p v-if="loading" class="mt-1 text-xs text-admin-theme-text-muted flex items-center">
