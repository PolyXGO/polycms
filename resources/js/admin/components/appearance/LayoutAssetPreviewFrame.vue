<template>
 ...
 v-if="iframeSrc"
 ref="iframeRef"
 class="absolute left-0 top-0 border-0 pointer-events-none bg-white dark:bg-gray-950 transition-opacity duration-300"
 ...
 <div
 v-else
 class="absolute inset-0 flex items-center justify-center px-4 text-center text-xs font-semibold uppercase tracking-[0.18em] text-admin-theme-text-muted"
 >
 {{ fallbackLabel }}
 </div>

 <div
 v-if="isLoading && (iframeSrc || html)"
 class="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-950/80 z-10"
 >
 <div class="h-6 w-6 animate-spin rounded-full border-2 border-admin-theme-primary border-t-transparent"></div>
 </div>
 </div>
</template>
...
const emit = defineEmits<{
 (e: 'thumbnail-generated', url: string): void;
}>();

const handleLoad = () => {
 isLoading.value = false;
 
 if (props.assetId && iframeRef.value) {
 // Wait a bit for fonts/images inside the iframe to render
 setTimeout(async () => {
 try {
 const iframeDocument = iframeRef.value?.contentDocument;
 if (!iframeDocument || !iframeDocument.body) return;
 
 const dataUrl = await toPng(iframeDocument.body, {
 cacheBust: true,
 pixelRatio: 0.5,
 skipFonts: true
 });
 
 const response = await axios.post(`/api/v1/layout-assets/${props.assetId}/thumbnail`, {
 preview_image_base64: dataUrl
 });
 
 if (response.data?.data?.preview_image) {
 emit('thumbnail-generated', response.data.data.preview_image);
 }
 } catch (error) {
...
<body class="${isDark.value ?'dark' :''}">
 <div class="layout-asset-preview-root prose max-w-none">${props.html ||''}</div>
 <script>
 (() => {
