<template>
 ...
 >
 <EditorPanelWrapper
 v-for="(panel, index) in visibleMainPanels"
 :key="panel.key"
 :panel="panel"
 ...
 >
 <EditorPanelWrapper
 v-for="(panel, index) in visibleSidebarPanels"
 :key="panel.key"
 :panel="panel"
 :component="resolveComponent(panel)"
 :collapsed="panel.collapsed ?? false"
...
const editorContext = inject(EditorContextKey, null);

const isPanelVisible = (panel: EditorPanelDefinition): boolean => {
  if (!editorContext || !editorContext.form || !editorContext.form.value) {
    return true;
  }
  const form = editorContext.form.value;
  const productType = form.type || 'product';
  const normalizedKey = (panel.key || '').toLowerCase();

  // Rule 1: Service Configuration panel is only applicable for service or digital products
  if (normalizedKey === 'service' || normalizedKey === 'productservicepanel') {
    return productType === 'service' || productType === 'digital';
  }

  // Rule 2: Product Variants panel is only applicable for variable products (or if variants/attributes exist)
  if (normalizedKey === 'variants' || normalizedKey === 'productvariantspanel') {
    const hasAttributes = Array.isArray(form._attributes) && form._attributes.length > 0;
    const hasVariants = Array.isArray(form._variants) && form._variants.length > 0;
    return productType === 'variable' || hasAttributes || hasVariants;
  }
...
const visibleMainPanels = computed(() => {
  return (panels.value.main || []).filter(isPanelVisible);
});

const visibleSidebarPanels = computed(() => {
  return (panels.value.sidebar || []).filter(isPanelVisible);
});
...
@media (min-width: 1024px) {
  ...
  /* Custom Webkit scrollbar for sidebar to ensure visibility and modern feel */
  .editor-panels__column--sidebar::-webkit-scrollbar {
    width: 6px;
  }

  .editor-panels__column--sidebar::-webkit-scrollbar-track {
    background: transparent;
  }

  .editor-panels__column--sidebar::-webkit-scrollbar-thumb {
  ...
  .editor-panels__column--sidebar::-webkit-scrollbar-thumb:hover {
    background-color: rgba(156, 163, 175, 0.6);
  }

  .dark .editor-panels__column--sidebar {
    scrollbar-color: rgba(156, 163, 175, 0.3) transparent;
  }

  .dark .editor-panels__column--sidebar::-webkit-scrollbar-thumb {
    background-color: rgba(156, 163, 175, 0.25);
  }

  .dark .editor-panels__column--sidebar::-webkit-scrollbar-thumb:hover {
