<template>
 ...
 <input v-model="state.heading" type="text" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-sm focus:ring-2 focus:ring-admin-theme-primary" placeholder="Ready to get started?">
 </div>

 <div class="form-group">
 <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Text</label>
 <textarea v-model="state.text" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-sm h-20 focus:ring-2 focus:ring-admin-theme-primary" placeholder="Description text..."></textarea>
 </div>

 <div class="form-group">
 <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Button Text</label>
 <input v-model="state.button_text" type="text" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-sm focus:ring-2 focus:ring-admin-theme-primary" placeholder="Get Started Free">
 </div>

 <div class="form-group">
 <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Button URL</label>
 <input v-model="state.button_url" type="text" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-sm focus:ring-2 focus:ring-admin-theme-primary" placeholder="/login">
 </div>
 </div>
...
watch(() => props.data, (newData) => {
 if (newData) syncState(newData);
}, { deep: true, immediate: true });

watch(() => props.modelValue, (newValue) => {
 syncState(newValue || props.data);
}, { deep: true, immediate: true });
</script>
