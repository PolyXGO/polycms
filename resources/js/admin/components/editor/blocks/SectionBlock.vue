<template>
 ...
 <div class="mb-4 border border-admin-theme-border bg-admin-theme-surface p-4 space-y-3">
 ...
 <select v-model="state.bg_type" class="w-24 bg-admin-theme-base border-admin-theme-border py-1.5 px-2 text-[10px] appearance-none focus:ring-1 focus:ring-admin-theme-primary">
 ...
 <select v-model="state.padding_class" class="w-full bg-admin-theme-base border-admin-theme-border p-2 text-xs">
 ...
 <input type="checkbox" v-model="state.overlay" class="form-checkbox h-3.5 w-3.5 text-admin-theme-primary rounded">
 ...
 <input v-model="state.bg_image" type="text" class="flex-1 bg-admin-theme-base border-admin-theme-border p-2 text-xs" placeholder="URL to image...">
 <button @click="openMediaPicker" class="px-3 py-2 bg-admin-theme-base text-xs hover:bg-admin-theme-base transition-colors">Pick</button>
 </div>
 </div>
 </div>
 </div>

 <div class="bg-admin-theme-primary/10/30 dark:bg-admin-theme-primary/100/5 border border-dashed border-indigo-100 dark:border-admin-theme-primary/20 p-4">
 <h4 class="text-xs font-bold uppercase tracking-widest text-admin-theme-primary dark:text-admin-theme-primary mb-4 flex items-center gap-2">
...
'is-dragging': isDraggingBlock(index),
 ...
 <div class="section-block-item__tools flex items-center gap-1">
 <span v-if="actionToastIndex === Number(index)" class="text-[10px] bg-admin-theme-primary text-admin-theme-primary-content px-1.5 py-0.5 rounded font-medium shadow-sm transition-all animate-pulse">
 {{ actionToastMsg }}
 </span>
 <button
 type="button"
 class="section-block-tool"
 title="Copy Options (Sao chép thuộc tính)"
 @click.stop="copyNestedOptions(Number(index))"
 >
 <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002-2v-2" /></svg>
 </button>
 <button
 type="button"
 class="section-block-tool"
 :disabled="!landingStore.copiedBlockData"
 :class="{'opacity-40 cursor-not-allowed': !landingStore.copiedBlockData}"
 title="Paste Options (Dán thuộc tính)"
 @click.stop="pasteNestedOptions(Number(index))"
 >
 ...
 type="button"
 class="section-block-tool"
 title="Duplicate Element (Nhân bản phần tử)"
 @click.stop="duplicateNestedBlock(Number(index))"
 >
...
const actionToastIndex = ref<number | null>(null);
const actionToastMsg = ref('');
let toastTimer: any = null;

const triggerActionToast = (idx: number, msg: string) => {
  actionToastIndex.value = idx;
  actionToastMsg.value = msg;
  if (toastTimer) clearTimeout(toastTimer);
  toastTimer = setTimeout(() => {
    actionToastIndex.value = null;
    actionToastMsg.value = '';
  }, 1500);
};

const copyNestedOptions = (index: number) => {
  const targetBlock = state.blocks[index];
  if (!targetBlock) return;
  landingStore.copyBlockOptions(targetBlock.type, targetBlock.data || {});
  triggerActionToast(index, 'Copied!');
};

const pasteNestedOptions = (index: number) => {
  const targetBlock = state.blocks[index];
  if (!targetBlock || !landingStore.copiedBlockData) return;
  const merged = landingStore.pasteBlockOptions(targetBlock.type, targetBlock.data || {});
  state.blocks[index].data = merged;
  triggerActionToast(index, 'Pasted!');
};

const duplicateNestedBlock = (index: number) => {
  const targetBlock = state.blocks[index];
  if (!targetBlock) return;
  const cloned = JSON.parse(JSON.stringify(targetBlock));
  state.blocks.splice(index + 1, 0, cloned);
  triggerActionToast(index + 1, 'Duplicated!');
};
...
const handleNestedPointerDown = (index: number, event: MouseEvent) => {
  ...
  if (target.closest('.showcase-img') || target.closest('[data-no-drag-intercept]')) {
...
watch(state, () => {
 ...
 if (JSON.stringify(payload) !== JSON.stringify(current)) {
    lastEmittedStateStr = JSON.stringify(payload);
    emit('update:modelValue', payload);
  }
}, { deep: true });

let lastEmittedStateStr = '';
let lastPropValStr = '';

watch(() => props.modelValue, (newValue) => {
  if (newValue) {
    const newValStr = JSON.stringify(newValue);
    if (newValStr === lastPropValStr) return;
    lastPropValStr = newValStr;
    if (newValStr === lastEmittedStateStr) return;
