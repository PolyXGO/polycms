<template>
 ...
 <select v-model="state.spacing" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-xs">
 ...
 <select v-model="state.style" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-xs">
 ...
 <input v-model="state.color" type="text" class="flex-1 bg-admin-theme-base border-admin-theme-border rounded-lg p-1 text-xs" placeholder="gray-200 or #HEX">
 </div>
 </div>
 </div>
...
const state = reactive({
 ...
 color: props.modelValue?.color || props.data?.color ||'var(--color-border, #e5e7eb)',
...
const previewStyle = computed(() => {
 ...
 return { borderColor:'var(--color-border, #e5e7eb)' };
});
...
watch(() => props.data, (newData) => {
 ...
 state.color = newData.color ||'var(--color-border, #e5e7eb)';
