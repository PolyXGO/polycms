<template>
    ...
    <div v-else class="features-grid-block-preview" :style="{ padding: state.padding }">
        ...
        <div class="grid gap-4" :style="{ gridTemplateColumns: `repeat(${state.columns}, 1fr)` }">
            <div v-for="(feature, index) in state.features" :key="index" class="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700" :style="{ textAlign: state.align }">
                <div class="w-10 h-10 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg flex items-center justify-center mb-3" :class="state.align === 'center' ? 'mx-auto' : (state.align === 'right' ? 'ml-auto' : '')">
