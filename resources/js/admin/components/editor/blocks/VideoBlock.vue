<template>
 ...
 type="text"
 class="flex-1 rounded-lg border border-admin-theme-border bg-admin-theme-base p-2 text-xs focus:ring-2 focus:ring-admin-theme-primary"
 placeholder="Pick an image from Media or paste a URL..."
 >
 <button
 type="button"
 class="rounded-lg bg-gray-100 px-3 py-2 text-xs font-medium transition-colors hover:bg-admin-theme-border"
 ...
 class="w-full rounded-lg border border-admin-theme-border bg-admin-theme-base p-2 text-sm"
 >
 ...
 <div class="form-group">
 <label class="mb-2 block text-[10px] font-bold uppercase tracking-wider text-gray-400">Preview Click Action</label>
 <select
 v-model="state.play_inline"
 class="w-full rounded-lg border border-admin-theme-border bg-admin-theme-base p-2 text-sm"
 >
 <option :value="false">Redirect to Video URL</option>
 <option :value="true">Play Video Inline</option>
 </select>
 </div>
...
const state = reactive({
 ...
 play_inline: readAttr('play_inline', true), // Default to playing inline to match expectations
});
...
function syncState(source: Record<string, any> | null | undefined) {
 ...
 state.play_inline = readSourceAttr(source,'play_inline', true);
