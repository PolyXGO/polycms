...
'is-dragging': isDraggingBlock(columnIndex, blockIndex),
 ...
 <div class="row-block-preview__item-tools flex items-center gap-1">
 <span v-if="actionToastNodeId === buildNestedNodeId(columnIndex, blockIndex)" class="text-[10px] bg-admin-theme-primary text-admin-theme-primary-content px-1.5 py-0.5 rounded font-medium shadow-sm transition-all animate-pulse">
 {{ actionToastMsg }}
 </span>
 <button
 type="button"
 class="row-block-preview__tool"
 title="Copy Options (Sao chép thuộc tính)"
 @click.stop="copyNestedOptions(columnIndex, blockIndex)"
 >
 <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002-2v-2" /></svg>
 </button>
 <button
 type="button"
 class="row-block-preview__tool"
 ...
 @click.stop="pasteNestedOptions(columnIndex, blockIndex)"
 >
 ...
 @click.stop="duplicateNestedBlock(columnIndex, blockIndex)"
 >
...
const actionToastNodeId = ref('');
const actionToastMsg = ref('');
let toastTimer: any = null;

const triggerActionToast = (nodeIdStr: string, msg: string) => {
  actionToastNodeId.value = nodeIdStr;
  actionToastMsg.value = msg;
  if (toastTimer) clearTimeout(toastTimer);
  toastTimer = setTimeout(() => {
    actionToastNodeId.value = '';
    actionToastMsg.value = '';
  }, 1500);
};

const copyNestedOptions = (columnIndex: number, blockIndex: number) => {
  const targetBlock = state.columns_data[columnIndex]?.blocks[blockIndex];
  if (!targetBlock) return;
  landingStore.copyBlockOptions(targetBlock.type, targetBlock.data || {});
  triggerActionToast(buildNestedNodeId(columnIndex, blockIndex), 'Copied!');
};

const pasteNestedOptions = (columnIndex: number, blockIndex: number) => {
  const targetBlock = state.columns_data[columnIndex]?.blocks[blockIndex];
  ...
  state.columns_data[columnIndex].blocks[blockIndex].data = merged;
  triggerActionToast(buildNestedNodeId(columnIndex, blockIndex), 'Pasted!');
};

const duplicateNestedBlock = (columnIndex: number, blockIndex: number) => {
  const targetBlock = state.columns_data[columnIndex]?.blocks[blockIndex];
  if (!targetBlock) return;
  const cloned = cloneBlock(targetBlock);
  state.columns_data[columnIndex].blocks.splice(blockIndex + 1, 0, cloned);
  triggerActionToast(buildNestedNodeId(columnIndex, blockIndex + 1), 'Duplicated!');
};
...
function handleNestedPointerDown(columnIndex: number, blockIndex: number, event: MouseEvent) {
  ...
  if (target.closest('.showcase-img') || target.closest('.mtxf-transform-overlay') || target.closest('[data-no-drag-intercept]')) {
...
watch(
  state,
  () => {
    // Skip emitting during media transform to prevent ProseMirror re-render
    // that would destroy TransformManager's overlay
    if ((window as any).isTransformingMedia || (window as any).isDraggingMediaGesture) return;
