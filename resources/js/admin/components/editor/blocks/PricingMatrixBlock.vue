<template>
 ...
 <div class="w-10 h-10 rounded-full bg-admin-theme-primary/15/50 flex items-center justify-center text-admin-theme-primary dark:text-admin-theme-primary">
 <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
 </svg>
 </div>
 <div>
 <h4 class="text-sm font-bold text-admin-theme-text">Pricing Matrix</h4>
 ...
 <select v-model="state.style" class="w-full bg-admin-theme-input-bg border-admin-theme-border rounded-lg p-2 text-sm">
 <option value="cards">Horizontal Cards</option>
 <option value="table">Comparison Table</option>
 <option value="list">Simple List</option>
 </select>
 </div>
 <div class="mt-4 p-3 bg-admin-theme-primary/10 rounded-md">
 <p class="text-[10px] text-admin-theme-primary italic">
 ...
 <div v-if="!hasPackages" class="p-6 bg-admin-theme-base rounded-lg border-2 border-dashed border-admin-theme-border flex items-center gap-4">
 <div class="w-10 h-10 rounded-full bg-admin-theme-primary/15/50 flex items-center justify-center text-admin-theme-primary dark:text-admin-theme-primary">
 <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
 </svg>
 ...
 <span class="block mt-1 text-admin-theme-primary dark:text-admin-theme-primary">Please define packages in Service Configuration.</span>
 </p>
 </div>
 </div>
 ...
 <div v-else-if="state.style ==='table'" class="pricing-table-container" style="overflow-x: auto; background: rgb(var(--admin-theme-surface)); border-radius: 1rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05), 0 2px 4px -1px rgba(0, 0, 0, 0.03); border: 1px solid #f1f5f9;">
 <table style="width: 100%; border-collapse: collapse; min-width: 600px; table-layout: fixed;">
 <thead>
 <tr style="background: rgb(var(--admin-theme-base));">
 ...
 <td style="padding: 1.25rem 1.5rem; font-weight: 500; color: #475569; background: rgb(var(--admin-theme-surface));">{{ key }}</td>
 <td v-for="(pkg, index) in packages" :key="index" style="text-align: center; padding: 1.25rem 1rem; background: rgb(var(--admin-theme-surface));">
 ...
 <i class="fas fa-times" style="color: rgb(var(--admin-theme-text-muted)); font-size: 1rem;"></i>
 </template>
 <template v-else>
 ...
 <span v-for="(val, key) in (pkg.capabilities ? Object.fromEntries(Object.entries(pkg.capabilities).slice(0, 3)) : {})" :key="key" style="font-size: 0.75rem; padding: 2px 10px; background: #f1f5f9; border-radius: 999px; color: rgb(var(--admin-theme-text-secondary)); font-weight: 500; white-space: nowrap;">
 {{ key }}
 </span>
 </div>
 </div>
 ...
 <div v-if="pkg.duration_value" style="font-size: 0.7rem; color: rgb(var(--admin-theme-text-muted)); text-transform: uppercase; font-weight: 700;">
