<template>
 ...
 <input v-model="state.src" type="text" class="flex-1 bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-xs focus:ring-2 focus:ring-admin-theme-primary" placeholder="https://...">
 <button @click="openMediaPicker" class="px-3 py-2 bg-admin-theme-base rounded-lg text-xs hover:bg-admin-theme-base">Pick</button>
 </div>
 </div>

 <div class="form-group">
 <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Alt Text</label>
 <input v-model="state.alt" type="text" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-xs" placeholder="Image description">
 </div>

 <div class="grid grid-cols-2 gap-4">
 <div class="form-group">
 <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Width</label>
 <select v-model="state.width" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-xs">
 ...
 <button v-for="align in ['left','center','right']" :key="align" @click="state.alignment = align" class="flex-1 p-2 rounded-lg border text-xs capitalize transition-all" :class="state.alignment === align ?'bg-admin-theme-primary text-admin-theme-primary-content border-admin-theme-primary' :'bg-admin-theme-surface border-admin-theme-border'">
 {{ align }}
 </button>
 </div>
 </div>
 ...
 <select v-model="state.border_radius" class="w-full bg-admin-theme-base border-admin-theme-border rounded-lg p-2 text-xs">
 ...
 <img v-if="state.src" :src="state.src" :alt="state.alt" class="w-full h-auto object-cover border border-admin-theme-border">
 <div v-else class="w-full aspect-video bg-admin-theme-base flex items-center justify-center text-gray-400">
