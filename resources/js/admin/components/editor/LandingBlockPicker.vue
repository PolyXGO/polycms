<template>
        ...
        <header class="p-4 border-b border-admin-theme-border flex items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            <button
              ...
              type="button"
              class="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-admin-theme-border text-admin-theme-text-muted transition-colors hover:border-admin-theme-primary hover:text-admin-theme-primary"
            ...
            <div>
              <h3 class="text-base font-bold text-admin-theme-text leading-none">
                {{ presetTargetBlock ? 'Choose Row Layout' : 'Add Landing Element' }}
              </h3>
              <p v-if="presetTargetBlock" class="mt-1 text-xs text-admin-theme-text-muted">Pick a row preset now. You can still change it later in the options panel.</p>
            </div>
          </div>

          <!-- Right side controls: Search Bar & Close button -->
          <div class="flex items-center gap-4">
      <!-- Search Bar (Only shown when not picking row layout) -->
      <div v-if="!presetTargetBlock" class="w-72 relative">
 ...
 />
  ...
  <div v-else>
    <div v-if="Object.keys(categorizedBlocks).length === 0" class="flex flex-col items-center justify-center py-12 text-admin-theme-text-muted">
 ...
 <h4 class="text-[10px] font-black text-admin-theme-primary uppercase tracking-[0.2em]">{{ getCategoryLabel(category) }}</h4>
 <div class="h-px flex-1 bg-gradient-to-r from-admin-theme-primary/20 to-transparent"></div>
 </div>
 ...
 <template v-else-if="block.key ==='fw_latest_products'">
 <span class="landing-picker-pattern__title"></span>
 <div class="landing-picker-pattern__grid">
 <span v-for="i in 4" :key="`product-${i}`" class="landing-picker-pattern__grid-card"></span>
 </div>
 </template>
 </div>
 ...
 <div class="flex items-center gap-3 mb-4 px-1">
 <h4 class="text-[10px] font-black text-admin-theme-primary uppercase tracking-[0.2em]">Reusable Parts</h4>
 <div class="h-px flex-1 bg-gradient-to-r from-admin-theme-primary/20 to-transparent"></div>
 </div>
 ...
 <footer class="p-4 bg-admin-theme-base/50 border-t border-admin-theme-border">
 <p class="text-xs text-center text-admin-theme-text-muted">
...
const categorizedBlocks = computed(() => {
  ...
  reusableParts.value.forEach((asset) => {
    ...
    if (asset.slug === 'latest-products') {
      managedReusablePatternKeys.add('fw_latest_products');
    }
  });
  ...
  return Object.fromEntries(
    Object.entries(grouped)
      .map(([category, blocks]) => [
        category,
        blocks.filter((block) => {
          if (managedReusablePatternKeys.has(block.key)) {
            return false;
          }
          if (query) {
            return block.label.toLowerCase().includes(query) || block.key.toLowerCase().includes(query);
          }
          return true;
        }),
      ])
...
.landing-picker-card:hover {
 ...
 box-shadow: 0 12px 24px rgb(var(--admin-theme-primary) / 0.08);
}
...
.landing-picker-card__preview {
 ...
 min-height: 8.8rem;
 border: 1px solid rgb(var(--admin-theme-border));
 background-color: rgb(var(--admin-theme-base));
 overflow: hidden;
 border-radius: 0.375rem;
}
...
.landing-picker-card__preview-fallback {
 ...
 color: rgb(var(--admin-theme-text-muted) / 0.72);
}
...
.landing-picker-card__badge {
 ...
 background: rgb(var(--admin-theme-primary) / 0.92);
 color: #fff;
 font-size: 0.58rem;
 font-weight: 900;
...
.landing-picker-pattern {
 ...
 gap: 0.55rem;
 border: 1px solid rgb(var(--admin-theme-border));
 background: rgb(var(--admin-theme-base) / 0.6);
 padding: 0.8rem;
 border-radius: 0.375rem;
}
...
.landing-picker-pattern__showcase-head {
 height: 0.9rem;
 background: rgb(var(--admin-theme-text) / 0.18);
 border-radius: 2px;
}
...
.landing-picker-pattern__cta-copy {
 height: 0.45rem;
 background: rgb(var(--admin-theme-text-muted) / 0.35);
 border-radius: 2px;
}
...
.landing-picker-pattern__showcase-btn {
 width: 4.4rem;
 height: 1.05rem;
 background: linear-gradient(135deg, rgb(var(--admin-theme-primary)) 0%, rgb(var(--admin-theme-primary) / 0.8) 100%);
}
...
.landing-picker-pattern__pricing-card--featured {
 border-color: rgb(var(--admin-theme-primary) / 0.6);
 background: linear-gradient(180deg, rgb(var(--admin-theme-primary) / 0.12) 0%, rgba(255, 255, 255, 0.95) 100%);
}
...
.landing-picker-pattern__list-item {
 display: block;
 height: 0.55rem;
 background: rgb(var(--admin-theme-text-muted) / 0.25);
 border-radius: 2px;
}

.landing-picker-pattern__highlight {
 height: 1.4rem;
 border-left: 3px solid rgb(var(--admin-theme-primary));
 background: rgb(var(--admin-theme-primary) / 0.08);
 border-radius: 2px;
}
...
.landing-picker-pattern__showcase-media {
 display: block;
 border: 1px solid rgb(var(--admin-theme-border));
 background: rgb(var(--admin-theme-text-muted) / 0.15);
 border-radius: 2px;
}
...
.landing-picker-pattern--hero_section {
 justify-content: center;
 background: linear-gradient(135deg, rgb(var(--admin-theme-primary) / 0.15) 0%, rgb(var(--admin-theme-base) / 0.4) 100%);
}

.landing-picker-pattern--showcase,
.landing-picker-pattern--what_you_get,
.landing-picker-pattern--features_grid {
 background: rgb(var(--admin-theme-base) / 0.4);
}
...
.custom-scrollbar::-webkit-scrollbar-thumb {
 background: rgb(var(--admin-theme-border));
 border-radius: 10px;
}
