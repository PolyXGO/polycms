<template>
 ...
 <span class="text-sm text-admin-theme-text-muted">Choose image</span>
 </template>
 </div>
 ...
 v-model="showTitle"
 size="lg"
 label="Show Heading on detail view"
 />
 </div>
 </div>
</template>
...
const showTitle = computed({
  get: () => context.form?.value?.show_title ?? true,
  set: (val) => {
    if (context.form?.value) {
      context.form.value.show_title = val;
    }
  },
});
</script>
