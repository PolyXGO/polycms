...
'is-dragging': isDraggingSelf,
 ...
 <div class="landing-block-actions flex items-center gap-1">
  <span v-if="actionToast" class="text-xs bg-admin-theme-primary text-admin-theme-primary-content px-2 py-0.5 rounded font-medium shadow-sm transition-all animate-pulse">
  {{ actionToast }}
  </span>
  <button type="button" @click.stop="handleCopyOptions" class="landing-block-btn text-admin-theme-text-muted hover:text-admin-theme-primary p-1 rounded transition-colors" title="Copy Options (Sao chép thuộc tính)">
  ...
  <button type="button" @click.stop="handlePasteOptions" :disabled="!landingStore.copiedBlockData" class="landing-block-btn text-admin-theme-text-muted hover:text-admin-theme-primary p-1 rounded transition-colors disabled:opacity-40 disabled:cursor-not-allowed" title="Paste Options (Dán thuộc tính)">
  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
  </button>
  <button type="button" @click.stop="handleDuplicate" class="landing-block-btn text-admin-theme-text-muted hover:text-admin-theme-primary p-1 rounded transition-colors" title="Duplicate Element (Nhân bản phần tử)">
  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
  </button>
  <button type="button" @click.stop="handleDelete" class="landing-block-delete text-red-500 hover:text-red-600 p-1 rounded transition-colors" title="Delete Block (Xóa)">
...
const handleDuplicate = () => {
  ...
  const newId = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : 'node_' + Date.now();
  const clonedData = JSON.parse(JSON.stringify(blockData.value || {}));
  ...
  triggerActionToast('Duplicated!');
};

const handleCopyOptions = () => {
  landingStore.copyBlockOptions(props.node.attrs.type, blockData.value);
  triggerActionToast('Options Copied!');
};

const handlePasteOptions = () => {
  if (!landingStore.copiedBlockData) return;
  const merged = landingStore.pasteBlockOptions(props.node.attrs.type, blockData.value);
  blockData.value = merged;
...
// blockData is derived reactively from node.attrs.data using a local ref
const localData = ref(JSON.parse(JSON.stringify(props.node.attrs.data || {})));

watch(
  () => props.node.attrs.data,
  (newData) => {
    const serializedNew = JSON.stringify(newData || {});
    if (JSON.stringify(localData.value) !== serializedNew) {
      localData.value = JSON.parse(serializedNew);
    }
  },
...
const blockData = computed({
  ...
  set: (newValue) => {
    ...
    if (JSON.stringify(localData.value) !== JSON.stringify(updated)) {
...
// Watch for Tiptap node data changes and sync back to the store
watch(
  () => props.node.attrs.data,
  (newData) => {
    if (newData) {
      const activeNodeId = landingStore.activeBlock?.nodeId;
      if (activeNodeId && activeNodeId.startsWith(`${nodeId.value}::`)) {
        // Nested block is active
        const path = activeNodeId.substring(nodeId.value.length + 2); // after "::"
        if (path.includes('column:')) {
          // RowBlock format: column:X:block:Y
          const colMatch = path.match(/column:(\d+)/);
          const blockMatch = path.match(/block:(\d+)/);
          if (colMatch && blockMatch) {
            const colIndex = parseInt(colMatch[1]);
            const blockIndex = parseInt(blockMatch[1]);
            const nestedBlock = newData.columns_data?.[colIndex]?.blocks?.[blockIndex];
            if (nestedBlock) {
              const currentStoreData = JSON.stringify(landingStore.activeBlock.data || {});
              const incomingData = JSON.stringify(nestedBlock.data || {});
              if (currentStoreData !== incomingData) {
                landingStore.setActiveBlockData(nestedBlock.data || {});
              }
            }
          }
        } else {
          // SectionBlock format: block:X
          const blockMatch = path.match(/block:(\d+)/);
          if (blockMatch) {
            const blockIndex = parseInt(blockMatch[1]);
            const nestedBlock = newData.blocks?.[blockIndex];
            if (nestedBlock) {
              const currentStoreData = JSON.stringify(landingStore.activeBlock.data || {});
              const incomingData = JSON.stringify(nestedBlock.data || {});
              if (currentStoreData !== incomingData) {
      ...
      } else if (isActiveInStore.value) {
        // Top-level block is active
        const currentStoreData = JSON.stringify(landingStore.activeBlock?.data || {});
        const incomingData = JSON.stringify(newData);
        if (currentStoreData !== incomingData) {
          landingStore.setActiveBlockData(newData);
        }
      }
    }
  },
