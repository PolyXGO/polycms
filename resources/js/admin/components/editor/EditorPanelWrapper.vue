<template>
    ...
    >
        ...
        <transition name="panel-collapse">
            <div v-show="!collapsed" class="editor-panel__body">
...
const handleDragOver = (event: DragEvent) => {
    ...
    const sourceIndex = Number(dataTransfer.getData('text/source-index') ?? -1);
    if (sourceArea !== props.area || sourceIndex === -1) {
    ...
    let targetIndex = shouldMoveBefore ? props.index : props.index + 1;

    if (!shouldMoveBefore && targetIndex > sourceIndex) {
        targetIndex -= 1;
    }

    if (targetIndex !== sourceIndex && targetIndex >= 0) {
        emit('reorder', { from: sourceIndex, to: targetIndex });
        dataTransfer.setData('text/source-index', String(targetIndex));
    }
};
