...
'editor-panel--locked': panel.movable === false,
 ...
 />
  ...
  <div class="editor-panel__title flex items-center gap-1.5">
  <span class="editor-panel__label">{{ panel.label }}</span>
  <HelpGuide 
    v-if="panel.key === 'service' || panel.key === 'productservicepanel'" 
    :title="t('Service & Package Configuration Guide')"
    :tooltip="t('Service & Package Configuration Guidelines')"
  >
    <div class="space-y-3 text-xs leading-relaxed">
      <p class="text-admin-theme-text-secondary border-b border-admin-theme-border/60 pb-2 font-medium">
        {{ t('Guide on configuring pricing packages, billing durations, presets, and software license policies in PolyCMS.') }}
      </p>
      <ul class="space-y-2.5 list-disc pl-4 text-admin-theme-text-secondary">
        <li>
          <strong class="text-admin-theme-text">{{ t('Quick Add Package Presets') }}:</strong>
          {{ t('Use daily, weekly, monthly, yearly, or lifetime presets to quickly fill standard service billing configurations.') }}
        </li>
        <li>
          <strong class="text-admin-theme-text">{{ t('Access Type') }}:</strong>
          {{ t('Choose Subscription for recurring/limited time billing or Permanent / Lifetime for a single one-time payment.') }}
        </li>
        <li>
          <strong class="text-admin-theme-text">{{ t('License Policy') }}:</strong>
          {{ t('Define key generation strategy: No License Key (free/open access), Per Seat (user seats limit), or Site License / Domain (website domain limits).') }}
        </li>
        <li>
          <strong class="text-admin-theme-text">{{ t('Max Activations') }}:</strong>
          {{ t('Specify max allowed domain or seat activations permitted per issued license key.') }}
        </li>
      </ul>
    </div>
  </HelpGuide>
  <HelpGuide 
    v-if="panel.key === 'pricing'" 
    :title="t('Pricing & Inventory Guide')"
    :tooltip="t('Pricing & Inventory Guidelines')"
  >
    <div class="space-y-3 text-xs leading-relaxed">
      <p class="text-admin-theme-text-secondary border-b border-admin-theme-border/60 pb-2 font-medium">
        {{ t('Comprehensive guide on managing product pricing, stock inventory, and product type behavior in PolyCMS.') }}
      </p>
      <ul class="space-y-2.5 list-disc pl-4 text-admin-theme-text-secondary">
        <li>
          <strong class="text-admin-theme-text">{{ t('Product Type') }}:</strong>
          {{ t('Select Physical (single item), Variable (multiple attributes), Service / License (digital licenses & subscriptions), or Digital Product (file downloads).') }}
        </li>
        <li>
          <strong class="text-admin-theme-text">{{ t('Price & Sale Price') }}:</strong>
          {{ t('Regular Price is the standard retail price. Sale Price sets a temporary discounted promotional price.') }}
        </li>
        <li>
          <strong class="text-admin-theme-text">{{ t('Stock Quantity & Status') }}:</strong>
          {{ t('Manage available inventory units and set stock status (Default Hide, In Stock, Out of Stock, On Backorder).') }}
        </li>
        <li>
          <strong class="text-admin-theme-text">{{ t('Customer Refund Policy') }}:</strong>
          {{ t('Enable customer refund requests for this product and configure the refund eligibility window in days.') }}
        </li>
      </ul>
    </div>
  </HelpGuide>
