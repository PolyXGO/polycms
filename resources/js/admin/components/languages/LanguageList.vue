<template>
 ...
 <table 
 class="min-w-full divide-y divide-admin-theme-border"
 >
 <thead class="bg-admin-theme-base">
 <tr>
 <th scope="col" class="w-10"></th>
 <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t('Language') }}</th>
 ...
 <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t('Default') }}</th>
 ...
 :class="[
 draggingIndex === index ?'opacity-40 border-2 border-dashed border-indigo-400 dark:border-admin-theme-primary scale-[0.98]' :'',
 dragOverIndex === index ?'bg-admin-theme-primary/10 border-indigo-200' :''
 ]"
 draggable="true"
 ...
 >
 <td class="pl-4 py-4 whitespace-nowrap cursor-move text-gray-400">
 ...
 <div class="ml-0">
 <div class="text-sm font-medium text-admin-theme-text">{{ lang.name }}</div>
 <div class="text-sm text-admin-theme-text-muted">{{ lang.native_name }}</div>
 </div>
 </div>
 </td>
 <td class="px-6 py-4 whitespace-nowrap">
 <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-admin-theme-base text-admin-theme-text-secondary">
 ...
 class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-admin-theme-primary"
 ...
 <button v-else @click="setDefault(lang)" class="text-admin-theme-primary hover:text-admin-theme-primary dark:text-admin-theme-primary dark:hover:text-admin-theme-primary text-xs">
 {{ t('Set Default') }}
 </button>
 </td>
 <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
 <div class="flex justify-end space-x-3">
 <router-link :to="`/admin/settings/languages/${lang.id}/translations`" class="text-admin-theme-primary hover:text-admin-theme-primary dark:text-admin-theme-primary" :title="t('Edit Translations')">
 ...
 <button @click="downloadLanguage(lang)" class="text-admin-theme-text-muted hover:text-gray-900 dark:text-admin-theme-text-muted" :title="t('Download Translations')">
 <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
 </svg>
 </button>
 <button @click="$emit('upload', lang)" class="text-admin-theme-text-muted hover:text-gray-900 dark:text-admin-theme-text-muted" :title="t('Upload Translations')">
 <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
import { useSortable } from'@/admin/composables/useSortable';
...
// Local copy of languages for sorting
...
// Drag and drop logic using the shared composable
const { 
 draggingIndex, 
 dragOverIndex, 
 handleDragStart, 
 ...
 handleDragEnd, 
 handleDrop 
} = useSortable(localLanguages, {
 onSort: async (newItems) => {
 try {
 await axios.post('/api/v1/languages/reorder', {
 ids: newItems.map(i => i.id)
 });
 emit('updated');
 dialog.success(t('Languages reordered'));
 } catch (e) {
 dialog.error(t('Failed to reorder languages'));
 // Revert on error
 localLanguages.value = [...props.languages];
 }
 }
});
