...
const sizeClasses = {
 ...
 label:'cursor-pointer text-[11px] font-bold uppercase tracking-[0.14em] text-admin-theme-text-muted',
 },
 md: {
 ...
 knobOff:'translate-x-0',
 label:'cursor-pointer text-sm font-medium text-admin-theme-text-secondary',
 },
 lg: {
 ...
 knobOn:'translate-x-5',
 knobOff:'translate-x-0',
 label:'cursor-pointer text-base font-medium text-admin-theme-text-secondary',
 },
} as const;
...
const toggleClasses = computed(() => {
 const baseClasses = `toggle-switch relative inline-flex shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-admin-theme-primary focus:ring-offset-2 focus:ring-offset-admin-theme-base ${currentSize.value.toggle}`;
 
 let classes = baseClasses;
 if (props.modelValue) {
 classes +=' bg-admin-theme-primary';
 } else {
 classes +=' bg-admin-theme-border';
 }
...
const knobClasses = computed(() => {
 const baseClasses = `toggle-knob pointer-events-none inline-block transform rounded-full shadow ring-0 transition duration-200 ease-in-out ${currentSize.value.knob}`;
 return props.modelValue
 ? `${baseClasses} bg-admin-theme-primary-content ${currentSize.value.knobOn}`
 : `${baseClasses} bg-white ${currentSize.value.knobOff}`;
});
