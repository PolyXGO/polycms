<template>
 <div
 class="form-tags-wrapper w-full px-3 py-2 border rounded-lg bg-admin-theme-input-bg text-admin-theme-text transition-all duration-200 flex flex-wrap gap-2 min-h-[42px]"
 :class="wrapperClasses"
 @click="focusInput"
 >
 <div v-for="(tag, index) in tags" :key="index" class="flex items-center gap-1 bg-admin-theme-primary/15 text-admin-theme-primary-hover dark:text-admin-theme-primary px-2 py-1 rounded text-sm">
 <span>{{ tag }}</span>
 <button type="button" @click.stop="removeTag(index)" class="hover:text-admin-theme-primary dark:hover:text-admin-theme-primary focus:outline-none">
 ...
 :readonly="readonly"
 class="flex-1 bg-transparent border-none focus:ring-0 p-0 text-sm text-admin-theme-text placeholder-gray-400 min-w-[120px] outline-none"
...
const wrapperClasses = computed(() => {
 ...
 return'border-admin-theme-border focus-within:border-admin-theme-primary focus-within:ring-1 focus-within:ring-indigo-500';
});
