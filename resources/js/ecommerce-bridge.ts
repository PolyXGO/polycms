...
(window as any).addToCart = async (item: any) => {
    ...
    // Try to auto-detect the button from window.event
    const e = window.event;
    let btn: HTMLElement | null = null;
    let originalPointerEvents = '';

    if (e && e.target) {
        btn = (e.target as HTMLElement).closest('button, .btn, a');
    }

    if (btn) {
        originalPointerEvents = btn.style.pointerEvents;
    ...
    try {
        ...
        if ((window as any).AppConfig?.redirectCartAfterAdd !== false) {
            ...
            // Pinia is reactive, so MiniCart will update automatically. No need to remount.
        }
    } catch (error) {
...
const mountMiniCart = () => {
    ...
    if (root) {
        ...
        miniCartApp = createApp({
            render: () => h(MiniCart)
        });
        
        miniCartApp.use(pinia);
