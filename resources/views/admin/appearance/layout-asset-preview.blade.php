...
<head>
    ...
    @php
        ...
        $activeThemes = \App\Models\Theme::where('is_active', true)->get();
    @endphp
    @foreach($activeThemes as $theme)
        @php $themeAssets = $templateResolver->resolveThemeAssets($theme->slug); @endphp
        @foreach($themeAssets['css'] as $cssPath)
            <link rel="stylesheet" href="{{ asset($cssPath) }}?v={{ time() }}">
        @endforeach
    @endforeach
...
<body class="{{ $isDark ? 'dark' : '' }}">
    <div class="layout-asset-preview-document prose max-w-none">{!! $contentHtml !!}</div>
