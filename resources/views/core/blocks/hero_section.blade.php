...
<section class="landing-hero {{ !$padding ? 'py-20' : '' }} px-4 bg-indigo-900 text-white text-center" style="{{ $styleAttr }}">
    <div class="max-w-4xl mx-auto">
        ...
        @if(!empty($attrs['button_text']))
            <a href="{{ $attrs['button_link'] ?? '#' }}" target="{{ get_link_target($attrs['button_link'] ?? '#') }}" class="inline-block bg-white text-indigo-900 px-8 py-4 rounded-full font-bold text-lg hover:bg-indigo-50 transition-all transform hover:scale-105 shadow-xl">
                {{ $attrs['button_text'] }}
            </a>
        @endif
    </div>
