                ...
                @if(!empty($button1Text) || !empty($button2Text))
                    <div class="showcase-block__actions">
                        @if(!empty($button1Text))
                            <a href="{{ $button1Url ?: '#' }}" target="{{ get_link_target($button1Url ?: '#') }}" class="showcase-block__button showcase-block__button--primary">
                                {{ $button1Text }}
                            </a>
                        @endif

                        @if(!empty($button2Text))
                            <a href="{{ $button2Url ?: '#' }}" target="{{ get_link_target($button2Url ?: '#') }}" class="showcase-block__button showcase-block__button--secondary">
                                {{ $button2Text }}
                            </a>
                        @endif
                    </div>
            ...
            <div class="showcase-block__media-wrap">
                <div class="showcase-block__media">
                    ...
                    @if(!empty($previewImage) && !empty($videoLink))
                        <a href="{{ $videoLink }}" class="showcase-block__play" target="{{ get_link_target($videoLink, '_blank') }}" rel="noopener noreferrer" aria-label="Play showcase video">
