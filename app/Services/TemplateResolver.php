...
class TemplateResolver
{
    ...
    protected function resolveFromTheme(string $viewName, string $themeSlug): ?string
    {
        ...
        if (View::exists($namespacedView)) {
            ...
            // Dynamically append this sub-theme's views to the finder.
            // addLocation() places it at the END of the search order, so:
            //   [0] Main theme (prepended) → wins for shared views (layouts, headers)
            //   [1] Core views (resources/views)
            //   [N] This sub-theme → fallback for @include calls within its templates
            // This avoids conflicts when multiple sub-themes are active.
