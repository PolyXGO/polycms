...
class CartService
{
    ...
    public function addItem(Cart $cart, int $productId, int $quantity = 1, ?int $variantId = null, array $metadata = []): CartItem
    {
        ...
        $existing = $existingItems->first(function ($item) use ($metadata) {
            $existingServiceId = $item->metadata['service_id'] ?? null;
     ...
     */
    public function updateItem(Cart $cart, int $itemId, int $quantity, $serviceId = 'ignore'): CartItem
    {
        $quantity = max(1, min($quantity, 99));
        ...
        if ($serviceId !== 'ignore') {
            ...
            $metadata['service_id'] = $serviceId !== null ? (int) $serviceId : null;
            
            // Allow plugins/filters to modify cart item metadata before updating
            $metadata = Hook::applyFilters('cart.item.metadata', $metadata, $item);
            
            $updateData['metadata'] = $metadata;
        }

        // Allow plugins/filters to modify the final update payload
        $updateData = Hook::applyFilters('cart.item.update_data', $updateData, $item);
    ...
    public function calculateTotals(Cart $cart): array
    {
        ...
        foreach ($cart->items as $item) {
            ...
            $serviceName = null;
            if ($serviceId) {
                $service = \App\Models\Ecommerce\ProductService::where('product_id', $product->id)
                    ->where('id', $serviceId)
                    ->first();
                if ($service) {
