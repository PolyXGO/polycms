...
class SettingsService
{
    ...
    protected ?array $defaultDefinitionsCached = null;
    protected ?array $activeLocalesCached = null;
    protected ?string $defaultLocaleCached = null;
    protected bool $isResolvingDefinitions = false;
     ...
     * @return array<string, array<string, mixed>>
     */
    public function getAllSettings(?string $locale = null): array
    {
        try {
            $cachedSettings = $this->rememberSettings($this->cacheKey, $this->cacheTtl, function () {
            ...
            foreach ($cachedSettings as $setting) {
                $dbSettingsMap[$setting->key] = $setting;
            }
            ...
            foreach ($cachedSettings as $setting) {
                ...
                if ($this->isLocalizedVariantKey($key)) {
                    continue;
                }
                ...
                // Skip orphaned settings (e.g., from deactivated themes or modules)
                if (!isset($grouped[$group][$key])) {
                    continue;
                }

                $existing = $grouped[$group][$key];
                ...
                $grouped[$group][$key] = array_merge($existing, [
                    'key' => $key,
                    'value' => $setting->value,
                    'type' => $existing['type'] ?? $setting->type ?? 'string',
                    'label' => $existing['label'] ?? $setting->label ?? $key,
                    'description' => $existing['description'] ?? $setting->description ?? '',
                    'group' => $group,
                ]);
            }
                ...
                foreach ($grouped as $group => &$items) {
                    foreach ($items as $key => &$item) {
                        if ($this->isTranslatableKey($key)) {
                            $localizedKey = "{$key}_{$targetLocale}";
                            if (isset($dbSettingsMap[$localizedKey])) {
                                $localizedVal = $dbSettingsMap[$localizedKey]->value;
                                if ($localizedVal !== null && $localizedVal !== '') {
                                    $item['value'] = $localizedVal;
                                }
                            }
                        }
                    }
     ...
     */
    public function getGroupSettings(string $group, ?string $locale = null): array
    {
        $allSettings = $this->getAllSettings($locale);
        ...
        try {
            ...
            if ($this->isTranslatableKey($key)) {
                ...
                if ($locale !== $defaultLocale) {
                    ...
                    if (array_key_exists($localizedKey, $settingsMap) && $settingsMap[$localizedKey] !== null && $settingsMap[$localizedKey] !== '') {
                        return $settingsMap[$localizedKey];
                    }
                }
            }

            $rawVal = array_key_exists($key, $settingsMap)
                ? $settingsMap[$key]
                : $default;

            return $this->castValue($key, $rawVal, $default);
        } catch (\Exception $e) {
            return $default;
        }
    }
    ...
    /**
     * Cast raw setting value to proper type (boolean, integer, float, string)
     */
    public function castValue(string $key, mixed $val, mixed $default = null): mixed
    {
        if ($val === null) {
            return $default;
        }
        ...
        $type = $definition['type'] ?? (is_bool($default) ? 'boolean' : (is_int($default) ? 'integer' : 'string'));

        if ($type === 'boolean') {
            if (is_bool($val)) return $val;
            if ($val === 'false' || $val === '0' || $val === 0 || $val === 'off' || $val === '') return false;
            if ($val === 'true' || $val === '1' || $val === 1 || $val === 'on') return true;
            return (bool) $val;
        }
    ...
    protected function getSettingsMap(): array
    {
        ...
        $this->settingsMap = $this->rememberSettings($this->cacheMapKey, $this->cacheTtl, function () {
            if (!$this->hasSettingsTable()) {
                return [];
            }
     ...
     * @param string $group
     * @param string $type
     * @param bool $dispatchHook
     * @param string|null $locale
     * @return Setting
     */
    public function set(string $key, $value, string $group = 'general', string $type = 'string', bool $dispatchHook = true, ?string $locale = null): Setting
    {
        $defaultLocale = $this->getDefaultLocale();
        $targetLocale = $locale ?? $defaultLocale;

        if ($this->isTranslatableKey($key) && $targetLocale !== $defaultLocale) {
            $key = "{$key}_{$targetLocale}";
        }
        ...
        if ($type === 'boolean') {
            $value = (is_bool($value) ? $value : in_array($value, ['true', '1', 1, 'on'], true)) ? '1' : '0';
        }
    ...
    {
        if ($this->defaultDefinitionsCached === null) {
            $this->defaultDefinitionsCached = $this->getDefaultDefinitions();
        }
        foreach ($this->defaultDefinitionsCached as $group => $items) {
            if (isset($items[$key])) {
                $definition = $this->normalizeDefinition($key, $items[$key]);
    ...
    {
        if ($this->defaultLocaleCached !== null) {
            return $this->defaultLocaleCached;
        }

        return $this->defaultLocaleCached = cache()->remember('default_language_code', 3600, function () {
            try {
    ...
    /**
     * Check if a setting key is translatable.
     *
     * @param string $key
     * @return bool
     */
    public function isTranslatableKey(string $key): bool
    {
        $definition = $this->getDefinition($key);
        if ($definition && isset($definition['translatable']) && $definition['translatable'] === true) {
            return true;
        }

        $fallbackTranslatableKeys = [
            'global_faqs_items',
            'global_tabs_items',
        ...
        if (in_array($key, $fallbackTranslatableKeys, true)) {
            return true;
        }

        if (str_ends_with($key, '_copyright')) {
            return true;
        }

        return (bool) Hook::applyFilters('settings.key.is_translatable', false, $key);
    }

    /**
     * Check if a setting key is a localized variant key (e.g. site_title_vi).
     *
     * @param string $key
     * @return bool
     */
    public function isLocalizedVariantKey(string $key): bool
    {
        $lastUnderscore = strrpos($key, '_');
        if ($lastUnderscore === false || $lastUnderscore === 0 || $lastUnderscore === strlen($key) - 1) {
            return false;
        }
        ...
        if (!in_array($suffix, $this->activeLocalesCached, true)) {
            return false;
        }

        return $this->isTranslatableKey($baseKey);
    }
     ...
     * @return void
     */
    public function setMultiple(array $settings, string $group = 'general', ?string $locale = null): void
    {
        $changedKeys = [];
        ...
        foreach ($settings as $key => $data) {
            if (is_array($data) && array_key_exists('value', $data)) {
                ...
                $this->set($key, $data, $group, $type, false, $locale);
            }
    ...
    {
        $this->forgetSettings($this->cacheKey);
        $this->forgetSettings($this->cacheMapKey);
        $this->settingsMap = null;
        $this->settingsTableExists = null;
    }

    protected function rememberSettings(string $key, int $ttl, \Closure $callback): mixed
    {
        try {
            // Force settings cache to always use local file store to avoid multi-tenant Redis cache conflicts
            return \Illuminate\Support\Facades\Cache::store('file')->remember($key, $ttl, $callback);
        } catch (\Throwable $e) {
            // Fallback: just compute and return the value directly without cache
            return $callback();
        }
    }

    protected function forgetSettings(string $key): void
    {
        try {
            \Illuminate\Support\Facades\Cache::store('file')->forget($key);
        } catch (\Throwable $e) {
            // Ignore failures on cache clearing
        }
    }

    /**
     * Get default socials settings structure
     *
     * @return array<string, array{key: string, value: mixed, type: string, label: string, description: string}>
     */
    public function getDefaultSocialsSettings(): array
    {
        return [
            'social_links' => [
                'key'         => 'social_links',
                'value'       => json_encode([
                    ['name' => 'Facebook', 'icon' => 'facebook', 'url' => 'https://www.facebook.com/polyxgoltd'],
                    ['name' => 'YouTube', 'icon' => 'youtube', 'url' => 'https://headrandom.com/GlU63s5D'],
                    ['name' => 'GitHub', 'icon' => 'github', 'url' => 'https://github.com/polyxgo'],
                    ['name' => 'Envato', 'icon' => 'envato', 'url' => 'https://codecanyon.net/user/polyxgo/portfolio']
                ]),
                'type'        => 'json',
                'label'       => 'Social Links JSON',
                'description' => 'List of social media profiles, icons and links.',
            ],
            'social_facebook' => [
                'key'         => 'social_facebook',
                'value'       => 'https://www.facebook.com/polyxgoltd',
                'type'        => 'string',
                'label'       => 'Facebook Profile URL',
                'description' => 'Link to your Facebook page or profile.',
            ],
            'social_youtube' => [
                'key'         => 'social_youtube',
                'value'       => 'https://headrandom.com/GlU63s5D',
                'type'        => 'string',
            ...
            'social_github' => [
                'key'         => 'social_github',
                'value'       => 'https://github.com/polyxgo',
                'type'        => 'string',
            ...
            'social_envato' => [
                'key'         => 'social_envato',
                'value'       => 'https://codecanyon.net/user/polyxgo/portfolio',
                'type'        => 'string',
                'label'       => 'Envato Profile URL',
                'description' => 'Link to your Envato/Codecanyon portfolio.',
            ],
            'social_twitter' => [
                'key'         => 'social_twitter',
                'value'       => '',
    ...
    public function getDefaultGeneralSettings(): array
    {
        return [
            'site_title' => [
                ...
                'type' => 'string',
                'label' => _l('Site Title'),
                'description' => _l('The name of your site'),
            ],
            'tagline' => [
                'key' => 'tagline',
                ...
                'type' => 'string',
                'label' => _l('Tagline'),
                'description' => _l('In a few words, explain what this site is about.'),
            ],
            'brand_logo' => [
                'key' => 'brand_logo',
                ...
                'type' => 'string',
                'label' => _l('Brand Logo'),
                'description' => _l('Upload a logo for your brand. If no logo is set, the brand name will be displayed instead.'),
            ],
            'brand_name' => [
                'key' => 'brand_name',
                'value' => 'POLYCMS',
                'type' => 'string',
                'label' => _l('Brand Name'),
                'description' => _l('Custom brand name to display when no logo is available. Defaults to "POLYCMS" if empty.'),
            ],
            'show_brand_label' => [
                'key' => 'show_brand_label',
                'value' => true,
                'type' => 'boolean',
                'label' => _l('Show Site Name next to Logo'),
                'description' => _l('When enabled, the brand name text will be displayed alongside the logo in the admin sidebar.'),
            ],
            'admin_email' => [
                'key' => 'admin_email',
                'value' => null,
                'type' => 'string',
                'label' => _l('Admin Email'),
                'description' => _l('Email address for the site administrator.'),
            ],
            'site_language' => [
                'key' => 'site_language',
                'value' => 'en',
                'type' => 'string',
                'label' => _l('Site Language'),
                'description' => _l('The default language for your site. Modules and themes can use this for localization.'),
            ],
            'site_language_direction' => [
                'key' => 'site_language_direction',
                'value' => 'ltr',
                'type' => 'string',
                'label' => _l('Front Site Language Direction'),
                'description' => _l('Text direction for the frontend site. This will be applied to the CSS direction property.'),
            ],
            'admin_topbar_dark_mode' => [
                'key' => 'admin_topbar_dark_mode',
                'value' => true,
                'type' => 'boolean',
                'label' => _l('Show Topbar Dark/Light Mode Toggle'),
                'description' => _l('Enable or disable the theme toggle button on the Admin Topbar.'),
            ],
            'site_icon' => [
                'key' => 'site_icon',
                'value' => '/assets/defaults/polycms-logo.png',
                'type' => 'string',
                'label' => _l('Site Icon'),
                'description' => _l('The icon/favicon for your site'),
            ],
            'timezone' => [
                'key' => 'timezone',
                'value' => 'UTC',
                'type' => 'string',
                'label' => _l('Timezone'),
                'description' => _l('Choose a city in the same timezone as you.'),
            ],
            'date_format' => [
                'key' => 'date_format',
                'value' => 'Y-m-d',
                'type' => 'string',
                'label' => _l('Date Format'),
                'description' => _l('The format date information is displayed.'),
            ],
            'time_format' => [
                'key' => 'time_format',
                'value' => 'H:i',
                'type' => 'string',
                'label' => _l('Time Format'),
                'description' => _l('The format time information is displayed.'),
            ],
            'week_starts_on' => [
                'key' => 'week_starts_on',
                'value' => '1', // Monday
                'type' => 'string',
                'label' => _l('Week Starts On'),
                'description' => _l('The day of the week the calendar week should start on.'),
            ],
            'session_lifetime' => [
                'key' => 'session_lifetime',
                'value' => 120,
                'type' => 'number',
                'label' => _l('Session Lifetime (minutes)'),
                'description' => _l('How long an admin session stays active before requiring re-login. Default: 120 minutes. Recommended: 480+ for heavy editing.'),
            ],
            'api_rate_limit' => [
                'key' => 'api_rate_limit',
                'value' => 300,
                'type' => 'number',
                'label' => _l('API Rate Limit (requests/minute)'),
                'description' => _l('Maximum API requests per minute per user. Increase if you experience unexpected logouts when editing many items. Default: 300.'),
            ],
        ];
    }
    ...
    public function getDefaultReadingSettings(): array
    {
        return [
            ...
            'reading_default_post_image' => [
                'key' => 'reading_default_post_image',
                'value' => null,
                'type' => 'image',
                'label' => 'Default Post Image',
                'description' => 'Fallback image used when a post does not have a featured image.',
            ],
    ...
    /**
     * Get the raw baseline setting definitions without hook filters applied
     */
    protected function getRawDefaultDefinitions(): array
    {
        return [
            'socials' => $this->getDefaultSocialsSettings(),
            'contacts' => [
                'contacts_recaptcha_enabled' => [
                    'key' => 'contacts_recaptcha_enabled',
                    'value' => false,
                    'type' => 'boolean',
                    'label' => 'Enable Google reCAPTCHA',
                    'description' => 'Enable Google reCAPTCHA protection for contact and newsletter forms.',
                ],
                'contacts_recaptcha_site_key' => [
                    'key' => 'contacts_recaptcha_site_key',
                    'value' => '',
                    'type' => 'string',
                    'label' => 'Google reCAPTCHA Site Key',
                    'description' => 'Your Google reCAPTCHA v2 site key.',
                ],
                'contacts_recaptcha_secret_key' => [
                    'key' => 'contacts_recaptcha_secret_key',
                    'value' => '',
                    'type' => 'string',
                    'label' => 'Google reCAPTCHA Secret Key',
                    'description' => 'Your Google reCAPTCHA v2 secret key.',
                ],
            ],
            ...
            'cache_optimization' => $this->getDefaultCacheOptimizationSettings(),
            'admin_appearance' => [
                'admin_theme' => [
                    'key' => 'admin_theme',
                    'value' => 'nebula',
                    'type' => 'string',
                    'label' => 'Admin Theme',
                    'description' => 'The theme used for the admin dashboard',
                ]
            ],
        ];
    }
    ...
    {
        if ($this->isResolvingDefinitions) {
            return $this->getRawDefaultDefinitions();
        }

        $this->isResolvingDefinitions = true;
        try {
            $defaults = $this->getRawDefaultDefinitions();
            $filtered = Hook::applyFilters('settings.defaults', $defaults, $this);
        } finally {
            $this->isResolvingDefinitions = false;
        }
    ...
    public function getDefaultMediaSettings(): array
    {
        ...
        return [
            ...
            'media_enable_lazy_loading' => ['key' => 'media_enable_lazy_loading', 'value' => true, 'type' => 'boolean'],
            ...
            'media_thumb_width' => ['key' => 'media_thumb_width', 'value' => 150, 'type' => 'integer'],
            'media_thumb_height' => ['key' => 'media_thumb_height', 'value' => 150, 'type' => 'integer'],
            'media_featured_width' => ['key' => 'media_featured_width', 'value' => 565, 'type' => 'integer'],
            'media_featured_height' => ['key' => 'media_featured_height', 'value' => 375, 'type' => 'integer'],
            'media_allowed_file_types' => [
                'key' => 'media_allowed_file_types', 
                'value' => '.png,.jpg,.jpeg,.pdf,.doc,.docx,.xls,.xlsx,.zip,.rar,.txt', 
                'type' => 'string',
    ...
    public function getDefaultEcommerceSettings(): array
    {
        return [
            ...
            'currencies' => [
                ...
                'type' => 'array',
                'label' => 'Currencies',
                'description' => 'List of store currencies',
                'section' => 'currency',
                'order' => 190,
            ],
            'currency_formatting_rules' => [
                'key' => 'currency_formatting_rules',
                'value' => [],
                'type' => 'json',
                'label' => 'Currency Formatting Rules',
                'description' => 'Auto-detect and API provider rules',
                'section' => 'currency',
                'order' => 200,
            ],
            'ecommerce_default_product_image' => [
                'key' => 'ecommerce_default_product_image',
                'value' => null,
                'type' => 'image',
                'label' => 'Default Product Image',
                'description' => 'Fallback image used when a product does not have a featured image.',
                'section' => 'shopping_experience',
                'order' => 30,
            ],
            'product_views_tracking_mode' => [
                'key' => 'product_views_tracking_mode',
                'value' => 'every_visit',
                'type' => 'select',
                'label' => 'Product Views Tracking Mode',
                'description' => 'Select how product page views are tracked and incremented.',
                'options' => [
                    ['label' => 'Every Visit (Page Refresh)', 'value' => 'every_visit'],
                    ['label' => 'Unique Session (24-hour Cookie per Product)', 'value' => 'unique_24h'],
                ],
                'section' => 'formatting',
                'order' => 210,
            ],
    ...
    /**
     * Get default cache optimization settings structure
     *
     * @return array<string, array{key: string, value: mixed, type: string, label: string, description: string}>
     */
    public function getDefaultCacheOptimizationSettings(): array
    {
        return [
            'polycms_cache_enabled' => [
                'key' => 'polycms_cache_enabled',
                'value' => 'yes',
                'type' => 'string',
                'label' => 'Enable System Cache',
                'description' => 'Toggle persistent caching across the entire application.',
            ],
            'cache_store' => [
                'key' => 'cache_store',
                'value' => 'file',
                'type' => 'string',
                'label' => 'Cache Store',
                'description' => 'Default store driver for caching',
            ],
            'session_driver' => [
                'key' => 'session_driver',
                'value' => 'file',
                ...
                'description' => 'Driver for managing user sessions',
            ],
            'queue_connection' => [
                ...
                'description' => 'Connection driver for background queues',
            ],
            'redis_host' => [
                'key' => 'redis_host',
                'value' => '127.0.0.1',
                'type' => 'string',
                'label' => 'Redis Host',
                'description' => 'Host address for Redis connection',
            ],
            'redis_port' => [
                'key' => 'redis_port',
                'value' => 6379,
            ...
            'redis_password' => [
                ...
                'description' => 'Authentication password for Redis connection',
            ],
            'redis_cache_db' => [
                'key' => 'redis_cache_db',
                'value' => 1,
                'type' => 'number',
                'label' => 'Redis Cache DB',
                'description' => 'Redis database index used for caching (Default: 1)',
            ],
            'redis_cache_prefix' => [
                'key' => 'redis_cache_prefix',
                'value' => 'polycms_cache_',
                'type' => 'string',
                'label' => 'Redis Cache Prefix',
                'description' => 'Prefix to separate cache keys for multiple sites on the same Redis server',
            ],
            'cache_preset' => [
                'key' => 'cache_preset',
                'value' => 'ultra_instant',
                'type' => 'string',
                'label' => 'Performance Optimization Preset',
                'description' => 'Preset combination for instant page loading.',
            ],
            'instant_prefetch_enabled' => [
                'key' => 'instant_prefetch_enabled',
                'value' => 'yes',
                'type' => 'string',
                'label' => 'Instant Prefetch on Hover',
                'description' => 'Prefetch target page HTML when user hovers mouse over links.',
            ],
            'speculation_rules_enabled' => [
                'key' => 'speculation_rules_enabled',
                'value' => 'yes',
                'type' => 'string',
                'label' => 'Speculation Rules Pre-rendering',
                'description' => 'Browser Speculation Rules API for background pre-rendering.',
            ],
            'response_html_cache_enabled' => [
                'key' => 'response_html_cache_enabled',
                'value' => 'yes',
                'type' => 'string',
                'label' => 'Server Response HTML Cache',
                'description' => 'Cache compiled HTML pages on server (compatible with File, Database, Redis stores).',
            ],
            'browser_http_cache_enabled' => [
                'key' => 'browser_http_cache_enabled',
                'value' => 'yes',
                'type' => 'string',
                'label' => 'Browser HTTP Cache Headers',
                'description' => 'Send Cache-Control Stale-While-Revalidate headers to browsers.',
            ],
            'response_cache_ttl' => [
                ...
                'description' => 'Time to live for server response HTML cache in seconds.',
            ],
            'page_cache_swr_ttl' => [
                'key' => 'page_cache_swr_ttl',
                'value' => 600,
                'type' => 'number',
                'label' => 'Stale-While-Revalidate TTL (Seconds)',
                'description' => 'Stale window in seconds during which stale HTML is served while revalidating.',
            ],
            'cache_query_param_strategy' => [
                'key' => 'cache_query_param_strategy',
                'value' => 'ignore_known_tracking',
                'type' => 'string',
                'label' => 'URL Query Parameter Strategy',
                'description' => 'Strategy for handling URL query parameters during cache key building.',
            ],
            'cache_ignored_query_params' => [
                'key' => 'cache_ignored_query_params',
                'value' => 'utm_source, utm_medium, utm_campaign, utm_term, utm_content, fbclid, gclid, msclkid, ttclid, ref, _ga, _gl',
                'type' => 'string',
                'label' => 'Ignored Tracking Parameters',
                'description' => 'Comma-separated tracking parameters or wildcards (e.g. utm_*, fbclid) stripped before cache key generation.',
            ],
            'database_cache_max_entry_kb' => [
                'key' => 'database_cache_max_entry_kb',
                'value' => 512,
                'type' => 'number',
                'label' => 'Database Cache Max Entry Size (KB)',
                'description' => 'Maximum allowed size in KB for a single database cache entry to prevent DB memory bloat.',
            ],
            'database_cache_max_total_mb' => [
                'key' => 'database_cache_max_total_mb',
                'value' => 512,
                'type' => 'number',
                'label' => 'Database Cache Total Capacity Limit (MB)',
                'description' => 'Maximum total database cache capacity limit in MB.',
            ],
            'cache_integrity_mode' => [
                'key' => 'cache_integrity_mode',
                'value' => 'sha256',
                'type' => 'string',
                'label' => 'Cache Entry Integrity Verification Mode',
                'description' => 'Integrity verification mode for cached data envelopes (sha256, hmac_sha256, disabled).',
            ],
            'durable_view_counter_enabled' => [
                'key' => 'durable_view_counter_enabled',
                'value' => 'yes',
                'type' => 'string',
                'label' => 'Decoupled Durable View Counter',
                'description' => 'Batch post/product view counter updates to post_view_buckets without invalidating cache.',
            ],
            'cache_allowlist_routes' => [
                'key' => 'cache_allowlist_routes',
                'value' => 'home, page.show, post.show, category.show, product.show, projects.show',
                'type' => 'string',
                'label' => 'Cached Route Allowlist',
                'description' => 'Comma-separated route names allowed for HTML page caching.',
            ],
            'cache_blacklisted_paths' => [
                'key' => 'cache_blacklisted_paths',
                'value' => 'admin, api, cart, checkout, account, logout, login, register, password, livewire',
                'type' => 'string',
                'label' => 'Bypassed Path Blocklist',
                'description' => 'Comma-separated path prefixes that must ALWAYS bypass page cache.',
            ],
        ];
    }
    ...
    protected function getDefaultThemeSettings(): array
    {
        return [
            ...
            'theme_reading_max_width' => [
                'value' => 768,
                'type' => 'number',
                'label' => 'Reading Max Width (px)',
                'description' => 'Maximum width for reading content like articles.',
                'input_props' => ['min' => 600, 'max' => 1200, 'step' => 10, 'suffix' => 'px'],
                'section' => 'layout',
                'section_label' => 'Layout & Components',
                'section_order' => 30,
                'category' => 'layout',
    ...
    protected function getDefaultPermalinkSettings(): array
    {
        return [
            ...
            'permalink_category_base' => [
                ...
                'type' => 'text',
                'label' => 'Category Base',
                'description' => 'Prefix for category archive pages (used for posts and products). Leave empty to serve categories at the root level.',
                'section' => 'taxonomies',
