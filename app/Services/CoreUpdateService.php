...
class CoreUpdateService
{
    ...
    public function performUpdate(string $zipPath, int $userId): SystemUpdateLog
    {
        ...
        $backupPath = null;
        $stagingDir = null;
        $sourceReplacementStarted = false;
        ...
        try {
            ...
            // A directory sync deletes live files first, so recovery must be available from this point.
            $sourceReplacementStarted = true;
            ...
            // Update app version in .env file
            $this->updateEnvVersion($validation['package_version']);

            // Step 9: Run database migrations
            $log->addStep('migrate', 'running', 'Running database migrations...');
            $migrateOutput = $this->runPendingMigrations('Core update');
            ...
            // Step 10.5: Sync Themes & Modules
            $log->addStep('sync_assets', 'running', 'Syncing themes and discovering modules...');
            try {
                // Discover modules
                app(\App\Services\ModuleManager::class)->discoverModules();
                
                // Sync themes
                app(\App\Services\ThemeManager::class)->sync();
                
                $log->addStep('sync_assets', 'success', 'Themes and modules synced successfully.');
            } catch (\Exception $e) {
                $log->addStep('sync_assets', 'failed', 'Asset sync failed: ' . $e->getMessage());
                Log::warning('Core update asset sync warning: ' . $e->getMessage());
            }

            // Step 11: Exit maintenance mode
            $log->addStep('maintenance_off', 'running', 'Exiting maintenance mode...');
            $this->callArtisanOrFail('up', [], 'Unable to exit maintenance mode');
            ...
            try {
                if ($sourceReplacementStarted && $backupPath !== null) {
                    $log->addStep('auto_rollback', 'running', 'Update failed. Restoring the core backup...');

                    try {
                        $this->restoreBackupFiles($backupPath);
                        $log->addStep('auto_rollback', 'success', 'Core files restored from backup. Database migrations are forward-only and were not reverted.');
                    } catch (\Throwable $rollbackError) {
                        $log->addStep('auto_rollback', 'failed', 'Automatic rollback failed: ' . $rollbackError->getMessage());
                        Log::critical('Automatic core update rollback failed: ' . $rollbackError->getMessage(), [
                ...
                Log::critical('Core update failure handling failed: ' . $recoveryError->getMessage(), [
                    'update_error' => $e->getMessage(),
                ]);
            } finally {
                // Never leave a failed update in maintenance mode, even if logging is unavailable.
                try {
                    $this->callArtisanOrFail('up', [], 'Unable to exit maintenance mode after update failure');
    ...
    public function rollback(string $backupPath, int $userId): SystemUpdateLog
    {
        ...
        try {
            ...
            $bypassSecret = Str::uuid()->toString();
            $this->callArtisanOrFail('down', ['--secret' => $bypassSecret], 'Unable to enable maintenance mode');
            ...
            // Update app version in .env file to target rollback version
            $this->updateEnvVersion($targetVersion);

            // Database migrations are forward-only. Restoring code must not run newer schema changes again.
            $log->addStep('migrate', 'success', 'Database migrations are not reverted during a code rollback.');
            ...
            try {
                ...
                Log::critical('Rollback failure handling failed: ' . $recoveryError->getMessage(), [
                    'rollback_error' => $e->getMessage(),
                ]);
            } finally {
                try {
                    $this->callArtisanOrFail('up', [], 'Unable to exit maintenance mode after rollback failure');
                } catch (\Throwable $upError) {
                    Log::error('Failed to exit maintenance mode after rollback failure: ' . $upError->getMessage());
                }
            }
    ...
    /**
     * Delete all core backup zip files to free disk space.
     */
    public function cleanBackups(): array
    {
        if (!File::isDirectory($this->backupDir)) {
            return ['deleted_count' => 0, 'freed_bytes' => 0, 'freed_display' => '0 B'];
        }
        ...
        // Also clean up any leftover temp files from updates
        $tempFiles = File::isDirectory($this->tempDir) ? File::files($this->tempDir) : [];
        ...
        // Clean up leftover staging directories
        if (File::isDirectory($this->tempDir)) {
            $tempDirs = File::directories($this->tempDir);
            foreach ($tempDirs as $dir) {
                $dirName = basename($dir);
                if (str_starts_with($dirName, 'core-update-') || str_starts_with($dirName, 'rollback-')) {
                    File::deleteDirectory($dir);
                }
            }
        }

        Log::info("Cleaned {$deletedCount} backup files, freed {$this->formatBytesForLog($freedBytes)}.");

        return [
            'deleted_count' => $deletedCount,
            'freed_bytes' => $freedBytes,
            'freed_display' => $this->formatBytesForLog($freedBytes),
        ];
    }
    ...
    /**
     * Run pending migrations and fail the caller when Artisan returns a non-zero status.
     *
     * Laravel's Artisan facade does not throw for every command failure, so callers must
     * inspect the exit status before treating a schema update as successful.
     */
    public function runPendingMigrations(string $context = 'Database migration'): string
    {
        return $this->callArtisanOrFail('migrate', ['--force' => true], $context . ' migrations');
    }

    /**
     * Execute an Artisan command and convert both exceptions and non-zero exit codes
     * into actionable update failures.
     */
    private function callArtisanOrFail(string $command, array $parameters, string $context): string
    {
        try {
            $exitCode = Artisan::call($command, $parameters);
            $output = trim(Artisan::output());
        } catch (\Throwable $e) {
            throw new \RuntimeException($context . ' failed: ' . $e->getMessage(), 0, $e);
        }

        if ($exitCode !== 0) {
            $details = $output !== '' ? ' ' . $output : '';
            throw new \RuntimeException($context . " failed with exit code {$exitCode}." . $details);
        }
    ...
    /**
     * Restore the core files contained in a backup archive without running migrations.
     * Schema migrations are normally additive and cannot safely be reversed automatically.
     */
    private function restoreBackupFiles(string $backupPath): void
    {
        $stagingDir = $this->tempDir . '/auto-rollback-' . time() . '-' . Str::lower(Str::random(8));
        $this->ensureDirectory($stagingDir);

        try {
            $zip = new ZipArchive();
            if ($zip->open($backupPath) !== true) {
                throw new \RuntimeException('Failed to open the automatic rollback backup.');
            }
            ...
            if (File::isDirectory($stagingDir . '/vendor')) {
                $this->syncVendor($stagingDir);
            }
    ...
    /**
     * Reject absolute and parent-traversal paths before extracting an update archive.
     */
    ...
    /**
     * Format bytes into human-readable string for log output.
     */
    ...
    /**
     * Update the APP_VERSION value in the .env file.
     */
    private function updateEnvVersion(string $newVersion): void
    {
        ...
        $pattern = '/^(APP_VERSION\s*=.*)$/m';

        if (preg_match($pattern, $content)) {
            $content = preg_replace($pattern, 'APP_VERSION=' . $newVersion, $content);
        } else {
    ...
    /**
     * Purge leftover temporary extraction folders and prune old backups to prevent disk space exhaustion.
     */
    private function purgeOldTempAndBackups(): void
    {
        try {
            // 1. Purge all temp core-update and auto-rollback folders
            if (File::isDirectory($this->tempDir)) {
                foreach (File::directories($this->tempDir) as $dir) {
                    $dirName = basename($dir);
                    if (str_contains($dirName, 'core-update-') || str_contains($dirName, 'auto-rollback-')) {
                        File::deleteDirectory($dir);
                    }
                }
            ...
            // 2. Prune old backups, keeping only the 2 most recent
            if (File::isDirectory($this->backupDir)) {
                $backups = File::files($this->backupDir);
                usort($backups, fn($a, $b) => $b->getMTime() <=> $a->getMTime());
                if (count($backups) > 2) {
                    for ($i = 2; $i < count($backups); $i++) {
                        File::delete($backups[$i]->getPathname());
                    }
                }
            }
        } catch (\Throwable $e) {
            Log::warning('purgeOldTempAndBackups warning: ' . $e->getMessage());
        }
    }
}
