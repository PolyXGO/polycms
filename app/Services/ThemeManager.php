...
class ThemeManager
{
    ...
    {
        return ResilientCache::remember("{$this->activeThemeCacheKey}.{$type}", 3600, function () use ($type) {
    ...
    {
        return ResilientCache::remember("{$this->activeThemeCacheKey}.main", 3600, function () {
    ...
    /**
     * Run pending theme migrations when a theme is activated.
     *
     * Themes that ship storage-backed features can include migrations in:
     * - themes/{slug}/database/migrations
     * - themes/{slug}/src/database/migrations
     */
    protected function runThemeMigrations(Theme $theme): void
    {
        $themePath = (string) ($theme->full_path ?? '');
        if ($themePath === '') {
            return;
        }

        $candidates = [
            $themePath . '/database/migrations',
            $themePath . '/src/database/migrations',
        ];
        ...
        foreach ($candidates as $migrationPath) {
            ...
            try {
                ...
                Log::warning("Theme migration warning for {$theme->slug}: " . $e->getMessage());
            }
    ...
    public function clearCache(): void
    {
        ...
        ResilientCache::forget("{$this->activeThemeCacheKey}.frontend");
        ResilientCache::forget("{$this->activeThemeCacheKey}.admin");
        ResilientCache::forget("{$this->activeThemeCacheKey}.main");
