    ...
    public function execute(Post $post, array $data, ?array $categoryIds = null, ?array $tagIds = null, ?array $metaFields = null): Post
    {
        return DB::transaction(function () use ($post, $data, $categoryIds, $tagIds, $metaFields) {
            ...
            if (!empty($data['content_raw'])) {
                ...
                // Reset stale raw editor JSON to null if updating content_html directly (e.g. from markdown deploy API)
                $data['content_raw'] = null;
            }

            // Avoid overwriting existing translation_group_id with null
            if (array_key_exists('translation_group_id', $data) && is_null($data['translation_group_id']) && !is_null($post->translation_group_id)) {
                unset($data['translation_group_id']);
            }
