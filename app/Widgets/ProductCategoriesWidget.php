...
class ProductCategoriesWidget
{
    public function render(WidgetInstance $instance): string
    {
        ...
        if ($hideEmpty) {
            $categories = $categories->filter(fn(ProductCategory $category) => ($category->products_count ?? 0) > 0);
        }
    ...
    /**
     * @param array<int|string, \Illuminate\Support\Collection<int, ProductCategory>> $grouped
     */
    ...
    protected function renderCategoryLink(ProductCategory $category, bool $displayCount): string
    {
        $url = theme_permalink_url('product_categories', $category->slug, 'single');
