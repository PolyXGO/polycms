...
class PaymentGatewayController extends Controller
{
    ...
    /**
     * Test PayPal connection with provided temporary credentials.
     */
    public function testPaypalConnection(Request $request): JsonResponse
    {
        $validated = $request->validate([
        ...
        $className = 'Modules\Polyx\PaypalGateway\Services\PaypalApiClient';
        if (!class_exists($className)) {
            return response()->json([
                'success' => false,
                'message' => 'PayPal Gateway module is not installed or enabled.',
            ], 400);
        }

        try {
            $client = new $className();
            $client->setCredentials($validated['client_id'], $validated['client_secret'], $validated['mode']);
            
            $modeLabel = $validated['mode'] === 'live' ? 'Live' : 'Sandbox';
            $token = $client->getAccessToken();

            if ($token) {
                return response()->json([
                    'success' => true,
                    'message' => "PayPal {$modeLabel} connection successful! Credentials are valid.",
                ]);
            }

            return response()->json([
                'success' => false,
                'message' => "Failed to obtain access token from PayPal {$modeLabel}. Please check your credentials.",
            ], 400);
            ...
            return response()->json([
                'success' => false,
                'message' => "PayPal {$modeLabel} connection failed: " . $e->getMessage(),
            ], 500);
        }
    }
}
