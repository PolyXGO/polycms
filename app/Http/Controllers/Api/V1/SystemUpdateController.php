...
class SystemUpdateController extends Controller
{
    ...
    public function execute(Request $request): JsonResponse
    {
        ...
        if (!\Illuminate\Support\Facades\Schema::hasTable('system_update_logs')) {
            try {
                $this->updateService->runPendingMigrations('Preparing the system update');
            } catch (\Throwable $e) {
                Log::error('Unable to prepare system update logging: ' . $e->getMessage());

                return response()->json([
                    'success' => false,
                    'message' => 'Update preparation failed: ' . $e->getMessage(),
                ], 500);
            }
        }
    ...
    public function migrate(Request $request): JsonResponse
    {
        ...
        try {
            $output = $this->updateService->runPendingMigrations('Manual database migration');
    ...
    public function latestLog(Request $request): JsonResponse
    {
        ...
        if (!\Illuminate\Support\Facades\Schema::hasTable('system_update_logs')) {
            try {
                $this->updateService->runPendingMigrations('Preparing update history');
            } catch (\Throwable $e) {
                Log::error('Unable to prepare update history: ' . $e->getMessage());

                return response()->json([
                    'success' => false,
                    'message' => 'Unable to load update history: ' . $e->getMessage(),
                ], 500);
            }
        }
    ...
    /**
     * DELETE /api/v1/system/backups
     * Delete all core backup zip files to free disk space.
     */
    public function cleanBackups(Request $request): JsonResponse
    {
        if ($denied = $this->ensureAdmin($request)) {
            return $denied;
        }
        ...
        try {
            $result = $this->updateService->cleanBackups();

            return response()->json([
                'success' => true,
                'message' => "Cleaned {$result['deleted_count']} backup(s), freed {$result['freed_display']}.",
                'data' => $result,
            ]);
        } catch (\Throwable $e) {
