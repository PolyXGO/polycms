...
class UserController extends Controller
{
    public function index(Request $request): UserCollection
    {
        ...
        // Exclude other admin accounts if logged in under a demo account
        $user = $request->user();
        if ($user && app(\App\Services\SettingsService::class)->get('demo_builder_restrictions_enabled', true)) {
            $isDemo = \Modules\Polyx\DemoBuilder\Models\DemoAccount::where('user_id', $user->id)->exists();
            if ($isDemo && $user->id !== 1) {
    ...
    public function store(StoreUserRequest $request): JsonResponse
    {
        ...
        Hook::doAction('user.created', $user, $data);
    ...
    public function update(UpdateUserRequest $request, User $user): JsonResponse
    {
        ...
        Hook::doAction('user.updated', $user, $data);
    ...
    public function destroy(Request $request, User $user): JsonResponse
    {
        ...
        Hook::doAction('user.deleted', $user);
    ...
    protected function syncRoles(User $user, array $roles): void
    {
        ...
        Hook::doAction('user.roles.syncing', $user, $filteredRoles);

        $user->syncRoles($filteredRoles);

        Hook::doAction('user.roles.synced', $user, $filteredRoles);
    }
