...
class MediaController extends Controller
{
    ...
    public function upload(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            'file' => [
                ...
                function ($attribute, $value, $fail) {
                    if ($value) {
                        $blocked = ['php', 'php3', 'php4', 'php5', 'php7', 'php8', 'phtml', 'phar', 'exe', 'sh', 'bat', 'cgi', 'pl', 'asp', 'aspx', 'jsp', 'vbs', 'ps1'];
                        ...
                        if (in_array($guessedExt, $blocked)) {
                            $fail('The actual file content type is not allowed for security reasons.');
                        }
                        
                        // 2. Setting-based allowlist
                        $allowedTypesStr = app(\App\Services\SettingsService::class)->get('media_allowed_file_types', '.png,.jpg,.jpeg,.pdf,.doc,.docx,.xls,.xlsx,.zip,.rar,.txt');
                        if (!empty(trim($allowedTypesStr))) {
                            $allowed = array_map('trim', explode(',', strtolower(str_replace('.', '', $allowedTypesStr))));
                            if (!in_array($extension, $allowed)) {
                                $fail('The uploaded file type (.' . $extension . ') is not allowed. Allowed types: ' . $allowedTypesStr);
                            }
                        }
                    }
                }
        ...
        $sanitizedFileName = $mediaService->prepareUploadFileName($file->getClientOriginalName(), $uploadPath, $diskName);
        $path = $file->storeAs($uploadPath, $sanitizedFileName, $diskName);
    ...
    /**
     * Regenerate thumbnails for all images (or a specific batch)
     */
    public function regenerateThumbnails(Request $request): JsonResponse
    {
        ...
        ], 'Thumbnails regenerated successfully.');
    }
