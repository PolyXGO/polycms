...
class WidgetInstanceController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $locale = $validated['locale'] ?? $request->input('locale') ?? app()->getLocale();
        $config = $this->normalizeConfig($validated['config'] ?? [], $definition);
        $order = (int) WidgetInstance::where('widget_area_id', $validated['widget_area_id'])->where('locale', $locale)->max('order') + 10;
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        if (isset($validated['widget_area_id']) && $validated['widget_area_id'] !== $instance->widget_area_id) {
            ...
            $instance->order = (int) WidgetInstance::where('widget_area_id', $instance->widget_area_id)->where('locale', $instance->locale)->max('order') + 10;
        }
