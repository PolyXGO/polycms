...
class ProductCategory extends Model
{
    ...
    public function getFrontendUrlAttribute(): string
    {
        ...
        $url = '/' . $base . '/' . $this->slug;
        return \App\Facades\Hook::applyFilters('product_category.frontend_url', $url, $this);
    }
    ...
    public function descendants(): Collection
    {
        ...
        return static::whereIn('id', $descendantIds)
            ->orderBy('depth')
            ->orderBy('order')
            ->orderBy('name')
            ->get();
    ...
    public function descendantIds(): array
    {
        $ids = [];
        if ($this->path) {
            $ids = static::where('path', 'like', $this->path . '/%')
                ->pluck('id')
                ->toArray();
        }
     ...
     */
    public static function getTree(?int $maxDepth = null, ?string $locale = null): Collection
    {
        $query = static::query();
