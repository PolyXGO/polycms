    ...
    protected function registerThemeViews(ThemeManager $themeManager): void
    {
        try {
            ...
            foreach ($allActiveThemes as $theme) {
                ...
                if ($theme->isMain()) {
                    $mainTheme = $theme;
                    $mainViewsPath = $viewsPath;
                    View::addNamespace('theme', [$viewsPath]);
                }
                // Sub-theme views are NOT prepended here.
                // TemplateResolver dynamically adds the specific sub-theme's views
                // at resolve time to avoid conflicts between multiple active sub-themes.
            }

            // Prepend main theme → highest priority for shared views (layouts, partials)
            if ($mainViewsPath) {
                View::getFinder()->prependLocation($mainViewsPath);
            }
