$baseDir = 'd:\PolyXGO\2026\PolyCMS\polycms\polycms\modules\Polyx'
$modules = Get-ChildItem -Path $baseDir -Directory | Where-Object { $_.Name -ne 'XemTuoiXongDat' } # XemTuoiXongDat might not be a standard module with module.json, let's check. Actually let's include it if it has module.json

foreach ($mod in $modules) {
    $moduleName = $mod.Name
    $moduleJsonPath = Join-Path $mod.FullName "module.json"
    
    if (-not (Test-Path $moduleJsonPath)) {
        Write-Host "Skipping $moduleName (no module.json)"
        continue
    }
    
    $deployScriptPath = Join-Path $mod.FullName "deploy.ps1"
    
    $scriptContent = @"
`$ErrorActionPreference = 'Stop'

# ========================================
# $moduleName Module Deploy Script
# ========================================
...
"@
    ...
    Set-Content -Path $deployScriptPath -Value $scriptContent -Encoding UTF8
    Write-Host "Created deploy.ps1 for $moduleName"
    
    # Run the deploy script
    Write-Host "Running deploy.ps1 for $moduleName..."
    & powershell.exe -ExecutionPolicy Bypass -File $deployScriptPath
}
