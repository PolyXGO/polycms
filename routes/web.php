...
Route::middleware('auth')->group(function () {
    ...
    Route::post('/account/addresses', [\App\Http\Controllers\Frontend\AddressController::class, 'store'])->name('account.addresses.store');
    Route::put('/account/addresses/{id}', [\App\Http\Controllers\Frontend\AddressController::class, 'update'])->name('account.addresses.update');
    Route::delete('/account/addresses/{id}', [\App\Http\Controllers\Frontend\AddressController::class, 'destroy'])->name('account.addresses.destroy');
    Route::patch('/account/addresses/{id}/default', [\App\Http\Controllers\Frontend\AddressController::class, 'setDefault'])->name('account.addresses.default');
});
...
Route::prefix('api/v1')->group(function () {
    ...
    Route::get('search/suggestions', SearchSuggestionController::class)->name('api.v1.search.suggestions');
...
$registerFrontendRoutes = function () use (
    $postsArchiveBase, $postsSingleBase, $productsArchiveBase, $productsSingleBase,
    $categoryBase, $postTagBase, $productTagBase, $authorBase,
    $productCategoryBase, $productBrandBase, $pagesBase, $pageSlugConstraint
) {
    ...
    Route::get('/' . $productsSingleBase . '/{slug}/preview', [FrontendProductController::class, 'preview'])
        ->where('slug', '^[^/]+$')
...
// Keep default language frontend URLs canonical and unprefixed.
if ($defaultLocale !== '') {
    Route::get('/{locale}/{path?}', function (string $locale, ?string $path = null) {
        ...
        ->where('locale', preg_quote($defaultLocale, '/'))
        ->where('path', '.*')
        ->name('language.default-prefix.redirect');
}

// 2. Register localized prefixed routes for active non-default locales first
if (!empty($activeLocales)) {
    $localeRegex = implode('|', $activeLocales);
    Route::prefix('{locale}')->where(['locale' => $localeRegex])->group($registerFrontendRoutes);
}

// 3. Register default unprefixed routes second
...
Route::middleware(['web'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::get('/licenses/{license}/download/{release}', [
        \App\Http\Controllers\Api\V1\LicenseController::class,
        'downloadAsAdmin'
    ])->name('licenses.download-as-admin');
